...
- `PickBurstScenarioLabelSmoke.tscn` guards all-unit smoke scenario selection for `mage.pick_burst` identities. When label-cap pressure includes both counterplay and peel-style evidence, pick-burst rows keep `burst` before generic `peel`, while non-pick-burst defensive rows preserve the existing `peel` preference.
