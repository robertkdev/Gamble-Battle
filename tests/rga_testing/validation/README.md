...
- Full Probe 6v6 (subject-as-slot substitution)
  ...
  - Targeted 6v6 scenes:
    ...
    - `tests/rga_testing/validation/RoleMatrixProbe6v6Kythera.tscn` - tank team_fortification/damage_reduction/debuff across counterplay, fortify, and burst contexts, with explicit goal-primary span checks for the current buff-uptime residual shape.
...
- Accepted-miss guard coverage smoke
  ...
  - Reads the regenerated `outputs/audit_playtest/rga_accepted_misses_2026_06_25/accepted_gap_kind_summary.csv` and `accepted_lower_level_fail_spans.csv`; fails if the current 3 accepted-miss gap kinds are not each mapped to validation scenes covering synthetic proof, scenario-pack coverage, and the focused live residual probe, if the accepted span count is not 3, or if the detailed residual rows differ from the current Kythera/Totem contract. Regenerate the export with `tests/rga_testing/tools/Export-AcceptedMisses.ps1` before running this smoke.
...
- `AcceptedMissGuardCoverageSmoke.tscn` is the executable coverage manifest for the current accepted-miss audit state. It expects the regenerated summary to contain 3 gap kinds and 3 accepted spans, verifies every exported gap kind maps to committed validation-scene coverage, verifies the detailed CSV still contains only the exact Kythera buff-uptime and Totem peel-save/interrupt rows, and verifies `docs/rga/accepted_miss_residual_audit_2026-06-26.md` names every current gap kind, affected unit, and metric label.
