...
- Pick-burst scenario-label smoke
  ...
  - Guards all-unit smoke scenario selection for `mage.pick_burst` identities: pick-burst rows keep `burst` before generic `peel` under label-cap pressure, non-pick-burst defensive rows preserve the existing `peel` preference, and mage `burst` resolves to the back-lane `pick_burst_window` pack.
