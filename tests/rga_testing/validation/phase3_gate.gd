...
func _run() -> void:
	...
	var extras: Dictionary = _parse_kv(args)

	var profile_path: String = _resolve_profile_path(String(extras.get("profile", "")))
	var base_path: String = String(extras.get("config", ""))
	...
	var run_dir: String = _run_dir_for(settings.out_path, settings.run_id)
	var telemetry_files: Array[String] = _collect_shard_files(run_dir)
...
func _validate_scenario_jobs(settings: RGASettings) -> bool:
	var builder: RGAScenarioBuilder = RGAScenarioBuilder.new()
	var catalog: RGAArchetypeCatalog = RGAArchetypeCatalog.new()
	var archetypes: Array[String] = catalog.list_ids()
	var intents: Variant = (settings.metadata.get("scenario_intents", []) if settings.metadata != null else [])
	if intents is String:
		var parsed: Variant = JSON.parse_string(intents)
		if typeof(parsed) == TYPE_ARRAY:
			intents = parsed
		else:
			intents = []
	var jobs: Array = builder.build(settings, intents)
	...
	for job in jobs:
		if not (job is DataModels.SimJob):
			continue
		var map_id: String = String(job.scenario_id)
		var arch_a: String = job.metadata.get("team_a_archetype", "") if job.metadata != null else ""
		var arch_b: String = job.metadata.get("team_b_archetype", "") if job.metadata != null else ""
		var key: String = "%s|%s|%s" % [map_id, arch_a, arch_b]
...
func _validate_provenance(run_dir: String, files: Array[String], run_id: String) -> bool:
	var ok: bool = true
	for file in files:
		var fa: FileAccess = FileAccess.open(file, FileAccess.READ)
		if fa == null:
