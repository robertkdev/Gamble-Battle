...
func _run() -> void:
	...
	var rows_written: int = pipeline.run_all(settings)
	if rows_written <= 0:
		printerr("Phase2Gate: pipeline produced no rows")
		success = false

	var run_dir: String = _run_dir_for(settings.out_path, settings.run_id)
	var rows: Array[DataModels.TelemetryRow] = _load_telemetry_rows(run_dir)
	if rows.size() == 0:
...
func _verify_event_families(rows: Array[DataModels.TelemetryRow]) -> bool:
	var required_caps: Array[String] = [TelemetryCapabilities.CAP_CC, TelemetryCapabilities.CAP_TARGETS, TelemetryCapabilities.CAP_MOBILITY]
	var required_kinds: Dictionary = {
		...
		if row.events != null and row.events.size() > 0:
			...
			for evt in row.events:
				if evt is Dictionary:
					var kind: String = String(evt.get("kind", ""))
					if kind != "":
...
func _run_invariants(rows: Array[DataModels.TelemetryRow]) -> bool:
	var ok: bool = true
	for row in rows:
		var issues: Array = RGAInvariants.validate(row)
		if issues.size() > 0:
			ok = false
...
func _run_golden_scenarios() -> bool:
	var script: GoldenScenarios = GoldenScenarios.new()
	var ok: bool = true
...
func _run_metric_tests() -> bool:
	var caps: PackedStringArray = TelemetryCapabilities.all_caps()
	var result: Dictionary = MetricRegistry.run_all(caps, {}, [])
...
func _load_telemetry_rows(run_dir: String) -> Array[DataModels.TelemetryRow]:
	...
	var dir: DirAccess = DirAccess.open(run_dir)
	if dir == null:
		return rows
	dir.list_dir_begin()
	while true:
		...
		if not name.ends_with(".jsonl"):
			continue
		var path: String = "%s/%s" % [run_dir, name]
...
func _dict_to_row(data: Dictionary) -> DataModels.TelemetryRow:
	var row: DataModels.TelemetryRow = DataModels.TelemetryRow.new()
...
func _dict_to_outcome(d: Dictionary) -> DataModels.EngineOutcome:
	var o: DataModels.EngineOutcome = DataModels.EngineOutcome.new()
	if d == null:
		return o
...
func _run_dir_for(base: String, run_id: String) -> String:
	...
	if rid.strip_edges() == "":
		rid = "default"
	var dir: String = "%s/run_%s" % [root, rid]
	_check_dir(dir)
	return dir
