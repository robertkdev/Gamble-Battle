...
const WATCHDOG_REAL_SECONDS: float = 45.0
...
func _run() -> void:
	...
	await _select_starter("bo")
	await _settle_frames(4)
	_force_repro_state()
	await _settle_frames(6)
	print("%s: forced_state=%s" % [REPRO_NAME, JSON.stringify(_diagnostic_state())])
	_set_planning_timer_safe()
	await _press_continue(false, "Bo C5R4 repro")
	var combat_seen: bool = await _wait_for_combat_active(3.0)
	_expect(combat_seen, "Bo C5R4 repro did not enter combat; state=%s" % JSON.stringify(_diagnostic_state()))
	if combat_seen:
		var resolved: bool = await _wait_for_preview_or_loss(WATCHDOG_REAL_SECONDS)
		if not resolved:
			_expect(false, "Bo C5R4 repro stalled past %.1fs; state=%s" % [WATCHDOG_REAL_SECONDS, JSON.stringify(_diagnostic_state())])
		else:
...
func _force_repro_state() -> void:
	...
	_expect(controller != null, "Bo C5R4 repro combat controller missing")
	if controller == null:
		return
	var manager: Variant = controller.get("manager")
	_expect(manager != null, "Bo C5R4 repro combat manager missing")
	if manager == null:
		return
	manager.player_team.clear()
	for unit_id: String in PLAYER_IDS:
		var unit: Unit = UnitFactory.spawn(unit_id)
		_expect(unit != null, "Bo C5R4 repro failed to spawn player unit %s" % unit_id)
		if unit != null:
	...
	for bench_index: int in range(BENCH_IDS.size()):
		...
		_expect(bench_unit != null, "Bo C5R4 repro failed to spawn bench unit %s" % BENCH_IDS[bench_index])
		if bench_unit != null:
