...
const TEAM_CAP: int = 8
const WATCHDOG_REAL_SECONDS: float = 90.0
...
func _run() -> void:
	...
	_expect(combat_seen, "Axiom C4R5 repro did not enter combat; state=%s" % JSON.stringify(_diagnostic_state()))
	if combat_seen:
		var resolved: bool = await _wait_for_preview_or_loss(WATCHDOG_REAL_SECONDS)
		if not resolved:
			_expect(false, "Axiom C4R5 repro stalled past %.1fs; state=%s" % [WATCHDOG_REAL_SECONDS, JSON.stringify(_diagnostic_state())])
		else:
...
func _force_repro_state() -> void:
	...
	if Shop != null:
		Shop.reset_run()
		for _index: int in range(max(0, SHOP_LEVEL - 1)):
			Shop.buy_xp()
	if Economy != null:
	...
	_expect(controller != null, "Axiom C4R5 repro combat controller missing")
	if controller == null:
		return
	var manager: Variant = controller.get("manager")
	_expect(manager != null, "Axiom C4R5 repro combat manager missing")
	if manager == null:
		return
	manager.player_team.clear()
	for unit_id: String in PLAYER_IDS:
		var unit: Unit = UnitFactory.spawn(unit_id)
		_expect(unit != null, "Axiom C4R5 repro failed to spawn player unit %s" % unit_id)
		if unit != null:
	...
	for bench_index: int in range(BENCH_IDS.size()):
		...
		_expect(bench_unit != null, "Axiom C4R5 repro failed to spawn bench unit %s" % BENCH_IDS[bench_index])
		if bench_unit != null:
			Roster.set_slot(bench_index, bench_unit)
	MirrorBoardStore.capture_boss_board(CHAPTER, manager.player_team)
...
func _start_repro_combat() -> void:
	...
	if Economy != null:
		...
		Economy.bet_changed.emit(Economy.current_bet)
	var controller: Variant = _combat_controller()
	_expect(controller != null, "Axiom C4R5 repro combat controller missing at start")
	if controller == null:
		return
	var manager: Variant = controller.get("manager")
	_expect(manager != null, "Axiom C4R5 repro combat manager missing at start")
	if manager == null:
		return
	if manager.has_method("start_stage"):
		manager.start_stage()
	else:
		_expect(false, "Axiom C4R5 repro combat manager cannot start_stage")
