...
# MovementService (Facade)
# Orchestrates movement update per frame:
#   forced move → buff block → slot step → clamp → collision resolve.
#
# Public API parity with the previous arena movement component so the engine
# and UI can keep calling the same method names:
# - configure(tile_size, player_pos, enemy_pos, bounds)
# - ensure_capacity(player_count, enemy_count)
# - update_movement(state, delta, target_resolver)
# - get_player_position/get_enemy_position
# - player_positions_copy/enemy_positions_copy
# - bounds_copy, tile_size
# Additions:
# - set_profiles(team, profiles)
# - notify_forced_movement(team, idx, vec, dur)
# - set_debug_log_frames(n)
...
var _profiles_player: Array[MovementProfile] = []
var _profiles_enemy: Array[MovementProfile] = []

# Debug helpers
var _debug_watch_players: Array[int] = [0]
var _debug_watch_enemies: Array[int] = [0]
var _dbg_prev_blended_p: Dictionary = {}
var _dbg_prev_blended_e: Dictionary = {}
var _dbg_stuck_frames_p: Dictionary = {}
var _dbg_stuck_frames_e: Dictionary = {}

func configure(tile_size: float, player_pos: Array, enemy_pos: Array, bounds: Rect2) -> void:
	# Filter to Vector2 only
	var p: Array[Vector2] = []
	for v in player_pos:
...
func set_buff_system(buff_system: BuffSystem) -> void:
	if buff_system != null:
...
# Read-only accessors for callers (engine/UI)
...
# Entry used by engine
func update_movement(state: BattleState, delta: float, target_resolver: Callable) -> void:
	_update_impl(state, delta, target_resolver)

func update(delta: float, state: BattleState, target_resolver: Callable) -> void:
	_update_impl(state, delta, target_resolver)

func _update_impl(state: BattleState, delta: float, target_resolver: Callable) -> void:
	if delta <= 0.0:
		return
	...
	# Precompute alive arrays for both teams
	...
	# Determine current targets for both teams (for slot assignment)
	...
	# Build per-target attacker groups
	var enemy_groups: Dictionary = {} # enemy_idx -> Array[int] of player indices
	...
	var player_groups: Dictionary = {} # player_idx -> Array[int] of enemy indices
	...
	# Compute slot destinations for each attacker
	...
	# Collect attempted step caps for collision capping
	...
	for i in range(state.player_team.size()):
		...
		if forced.has_active("player", i):
			...
			# Diagnostic only: within enemy range (do not stop here)
			...
			# Local separation from nearby allies (graceful avoidance)
			...
			# Blend seek and separation; keep forward bias toward target
			var w_seek: float = max(0.0, tuning.seek_weight) # e.g. 1.0
			var w_sep: float = max(0.0, tuning.separation_weight) # e.g. 1.25..1.5
			...
			# forward guard: attenuate separation if it flips heading
			...
			# Arrival toward slot (not enemy). Robust and monotonic.
			var slow_r: float = max(4.0, radius * max(0.0, tuning.arrival_slow_radius_factor))
			var stop_r: float = min(slow_r * 0.5, max(1.0, radius * max(0.0, tuning.arrival_stop_radius_factor)))
			if stop_r >= slow_r:
				stop_r = max(1.0, slow_r * 0.5)

			var dist_to_slot: float = cur.distance_to(slot_pos)
			var arrive_t: float = 1.0
			if dist_to_slot < slow_r:
				var x: float = clampf((dist_to_slot - stop_r) / max(0.001, slow_r - stop_r), 0.0, 1.0)
				arrive_t = x * x * (3.0 - 2.0 * x) # smoothstep

			var min_speed: float = max(0.0, tuning.arrival_min_speed_factor) # keep >0 to avoid stall
			var speed_factor: float = (0.0 if dist_to_slot <= stop_r else max(min_speed, arrive_t))

			var move_dist: float = max(0.0, u.move_speed) * max(0.0, tuning.speed_scale) * max(0.0, delta) * speed_factor
			var step_cap: float = min(move_dist, dist_to_slot)
			step = blended * step_cap

			# Debug per-unit step composition for watched units + diagnostics
			if Debug.enabled and data.debug_log_frames > 0 and (_debug_watch_players.is_empty() or _debug_watch_players.has(i)):
				var ds: float = dist_to_slot
				var dt: float = cur.distance_to(tpos)
				var sep_mag: float = sep.length()
				var sep_dir_dot: float = dir_seek.dot(sep_dir) if sep_dir != Vector2.ZERO else 0.0
				print("[Step] team=player idx=", i, " cur=", cur, " tgt=", tgt_idx, " tpos=", tpos,
					  " slot=", slot_pos, " ds=", ds, " dt=", dt)
				print("[Diag] team=player idx=", i, " slow_r=", slow_r, " stop_r=", stop_r,
					  " min_speed=", min_speed, " speed_factor=", speed_factor,
					  " within_enemy=", within_enemy, " within_slot=", (ds <= stop_r))
				print("[Step] seek=", dir_seek, " sep=", sep, " sep_mag=", sep_mag,
					  " neighbors=", neigh_count, " dot_seek_sep=", sep_dir_dot)
				print("[Step] blended=", blended, " move_cap=", move_dist, " step_len=", step.length())

				# Oscillation / stuck
				var prev_b: Vector2 = _dbg_prev_blended_p.get(i, Vector2.ZERO)
				if prev_b != Vector2.ZERO and blended != Vector2.ZERO:
					var ang_prev: float = atan2(prev_b.y, prev_b.x)
					var ang_now: float = atan2(blended.y, blended.x)
					var diff_ang: float = abs(ang_now - ang_prev)
					if diff_ang > PI:
						diff_ang = TAU - diff_ang
					if diff_ang > deg_to_rad(120.0):
						print("[Osc] team=player idx=", i, " angle_prev=", ang_prev, " angle_now=", ang_now)
				_dbg_prev_blended_p[i] = blended
				var stuck_f: int = int(_dbg_stuck_frames_p.get(i, 0))
				if step.length() < 0.01:
					stuck_f += 1
				else:
					stuck_f = 0
				_dbg_stuck_frames_p[i] = stuck_f
				if stuck_f >= 15 and neigh_count > 0:
					print("[Stuck] team=player idx=", i, " frames=", stuck_f, " neighbors=", neigh_count, " ds=", ds)
		...
		if Debug.enabled and data.debug_log_frames > 0:
			print("[Move] team=player i=", i, " pos=", cur, " tgt=", tgt_idx, " tpos=", tpos, " step=", step, " new=", new_pos)
	...
	for j in range(state.enemy_team.size()):
		...
		if forced.has_active("enemy", j):
			...
			# Blend seek and separation with forward guard
			...
			# Arrival toward slot
			var slow_r2: float = max(4.0, radius * max(0.0, tuning.arrival_slow_radius_factor))
			var stop_r2: float = min(slow_r2 * 0.5, max(1.0, radius * max(0.0, tuning.arrival_stop_radius_factor)))
			if stop_r2 >= slow_r2:
				stop_r2 = max(1.0, slow_r2 * 0.5)

			var dist_to_slot2: float = cur_e.distance_to(slot_pos2)
			var arrive_t2: float = 1.0
			if dist_to_slot2 < slow_r2:
				var x2: float = clampf((dist_to_slot2 - stop_r2) / max(0.001, slow_r2 - stop_r2), 0.0, 1.0)
				arrive_t2 = x2 * x2 * (3.0 - 2.0 * x2)

			var min_speed2: float = max(0.0, tuning.arrival_min_speed_factor)
			var speed_factor2: float = (0.0 if dist_to_slot2 <= stop_r2 else max(min_speed2, arrive_t2))

			var move_dist2: float = max(0.0, e.move_speed) * max(0.0, tuning.speed_scale) * max(0.0, delta) * speed_factor2
			var step_cap2: float = min(move_dist2, dist_to_slot2)
			step2 = blended2 * step_cap2

			# Debug per-unit step composition for watched units + diagnostics
			if Debug.enabled and data.debug_log_frames > 0 and (_debug_watch_enemies.is_empty() or _debug_watch_enemies.has(j)):
				var ds2: float = dist_to_slot2
				var dt2: float = cur_e.distance_to(tpos2)
				var sep_mag2: float = sep2.length()
				var sep_dir_dot2: float = dir_seek2.dot(sep_dir2) if sep_dir2 != Vector2.ZERO else 0.0
				print("[Step] team=enemy idx=", j, " cur=", cur_e, " tgt=", tgt_idx2, " tpos=", tpos2,
					  " slot=", slot_pos2, " ds=", ds2, " dt=", dt2)
				print("[Diag] team=enemy idx=", j, " slow_r=", slow_r2, " stop_r=", stop_r2,
					  " min_speed=", min_speed2, " speed_factor=", speed_factor2,
					  " within_enemy=", within_enemy2, " within_slot=", (ds2 <= stop_r2))
				print("[Step] seek=", dir_seek2, " sep=", sep2, " sep_mag=", sep_mag2,
					  " neighbors=", neigh_count2, " dot_seek_sep=", sep_dir_dot2)
				print("[Step] blended=", blended2, " move_cap=", move_dist2, " step_len=", step2.length())

				# Oscillation / stuck
				var prev_b2: Vector2 = _dbg_prev_blended_e.get(j, Vector2.ZERO)
				if prev_b2 != Vector2.ZERO and blended2 != Vector2.ZERO:
					var ang_prev2: float = atan2(prev_b2.y, prev_b2.x)
					var ang_now2: float = atan2(blended2.y, blended2.x)
					var diff_ang2: float = abs(ang_now2 - ang_prev2)
					if diff_ang2 > PI:
						diff_ang2 = TAU - diff_ang2
					if diff_ang2 > deg_to_rad(120.0):
						print("[Osc] team=enemy idx=", j, " angle_prev=", ang_prev2, " angle_now=", ang_now2)
				_dbg_prev_blended_e[j] = blended2
				var stuck_f2: int = int(_dbg_stuck_frames_e.get(j, 0))
				if step2.length() < 0.01:
					stuck_f2 += 1
				else:
					stuck_f2 = 0
				_dbg_stuck_frames_e[j] = stuck_f2
				if stuck_f2 >= 15 and neigh_count2 > 0:
					print("[Stuck] team=enemy idx=", j, " frames=", stuck_f2, " neighbors=", neigh_count2, " ds=", ds2)
		...
		if Debug.enabled and data.debug_log_frames > 0:
			print("[Move] team=enemy j=", j, " pos=", cur_e, " tgt=", tgt_idx2, " tpos=", tpos2, " step=", step2, " new=", new_pos_e)

	# Resolve collisions (post-step), cap correction by per-unit step length
	collider.resolve(
