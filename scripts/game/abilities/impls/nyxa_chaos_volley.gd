...
func cast(ctx: AbilityContext) -> bool:
    ...
    # Increment persistent bonus arrows, capped at 3 (total shots = 1 + bonus)
    ...
    # Beyond the 3rd cast, stack damage bonus
    ...
    var dmg_bonus_msg: String = (" (+%d dmg)" % int(meta["damage_bonus"])) if int(meta["damage_bonus"]) > 0 else ""
