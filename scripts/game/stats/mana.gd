...
static func gain_on_attack(state: BattleState, team: String, index: int, u: Unit, ability_system = null, buff_system = null) -> Dictionary:
	...
	if ability_system != null and int(u.mana_max) > 0 and int(u.mana) >= int(u.mana_max):
		...
		# If we attempted a cast during another cast chain, defer it to the next tick
		if not result.cast and String(cast_res.get("reason", "")) == "in_cast":
			if ability_system.has_method("schedule_event"):
				ability_system.schedule_event("autocast_deferred", team, index, 0.0)
	return result
