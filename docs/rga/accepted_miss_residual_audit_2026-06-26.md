...
| Teller | The design doc defines Margin Call as line shots at the two furthest enemies with overflow damage and possible extra shots through Exile. The `sustained_dps` goal expects primary consistent front-to-back damage with uptime-adjusted DPS over 6-15 seconds, position penalty, damage mix, and deaths per fight. The doc also notes sustained DPS can be biased by early burst without windowed throughput. | The current direct team-damage-share residual should not be tuned blindly. Teller may need the sustained throughput kernel/windowed evidence before threshold changes, or a design review around whether his long-range burst/overflow kit is meant to stay `marksman.sustained_dps`. |
