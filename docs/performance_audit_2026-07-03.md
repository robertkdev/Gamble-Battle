...
1. Combat movement is the primary optimization surface.
   ...
   - `PerfMovementPhases.tscn` now proves `SlotStrategy.assign_for_target()` / slot assignment dominates the 12v12 movement profile at roughly `88-90%` of measured movement time. Slot allocation, range-dictionary cleanup, and inline circular-distance cleanup helped measured movement samples, but slot assignment remains the next primary optimization surface. Preserve real-combat signatures; direct Hungarian assignment changed behavior, Hungarian-bounded DP was slower in the real profiler, lower-bound DP pruning was slower, DP open-column caching was slower, and greedy-bounded exact DP also regressed real movement profiling.
