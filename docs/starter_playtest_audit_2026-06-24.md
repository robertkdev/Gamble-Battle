...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-25 after the latest accepted-miss export refresh with `gap_kinds=43`, `accepted_spans=80`, `mapped_gap_kinds=43`, and `errors: []`.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the 2026-06-25 saved report parser found 80 subject-side accepted lower-level `FAIL` spans across 21/22 reports after filtering opponent-side aggregate labels, suppressing non-applicable goal ramp and soft-peel hard-peel diagnostics, and adding live counterplay scenario rows. Latest accepted-miss export refresh still reports 80 spans across 43 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` passes. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
Latest refresh after the current audit follow-ups: a new MCP run wrote fresh JSON reports for all 22 current units between 2026-06-25 15:59 and 16:02 local time. Direct JSON verification found every current role, primary-goal, and approach verdict at `PASS`, and `get_debug_output()` returned `errors: []`. `Export-AcceptedMisses.ps1` regenerated the artifact with `reports=22`, `spans=80`, `ramp_spans=3`, and `non_ramp_goal_ramp=0`; `AcceptedMissGuardCoverageSmoke.tscn` then printed `gap_kinds=43 accepted_spans=80 mapped_gap_kinds=43` and `PASS` with `errors: []`.
