...
- `tests/visual/AllStarterMainFlowSmoke.tscn`: `AllStarterMainFlowSmoke: PASS starters=12 first_shop=11 retry=1 deployed=11 second_resolved=11`; rerun on 2026-06-26 with `errors: []`, validating every current level-1 starter through the real Main-scene forced opener, first-shop buy/deploy, and second-fight loop while preserving Axiom as the one expected opener retry outlier and asserting every guarded first shop starts with a known advancing helper.
- `outputs/audit_playtest/AllStarterMainFlowAudit.tscn`: `AllStarterMainFlowAudit: OK starters=12`; rerun on 2026-06-26 with `errors: []` after the expanded first-slot starter-aware first-shop guard. The current default first-card replay advanced all 11 first-shop starters beyond Stage 2, had no held Stage 2 default-click lines, and kept Axiom as the 2-gold retry starter.
- `tests/visual/AxiomRetryChoiceQualitySmoke.tscn`: `AxiomRetryChoiceQualitySmoke: PASS trials=5 advanced=5`; added and run on 2026-06-26 with `errors: []`, forcing Axiom's configured retry helpers through the real Main-scene buy/deploy/retry-fight path and proving Bonko, Grint, Sari, Morrak, and Berebell all advance beyond Stage 1.
...
- `tests/visual/AxiomRetryEconomySmoke.tscn`: `AxiomRetryEconomySmoke: OK`; rerun on 2026-06-26 with `errors: []`, validating the support-starter retry economy after a forced opener defeat, configured slot-0 retry helper, helper purchase/deploy guidance, bench highlight clearing, and retry-fight progression back into a full Stage 2 planning shop.
...
- The current committed validation now includes an automated Main-flow replay for all 12 current starters after the cost-tier and stage/reward changes, but not a fresh human real-window/screenshot replay of every starter.
...
| All current starters through first Main-flow loop | `AllStarterMainFlowSmoke` is now committed validation for all 12 current starters: Axiom retry plus 11 first-shop buy/deploy/second-fight paths through the real Main scene. The smoke now also asserts Bo, Bonko, Cashmere, Korath, Mortem, Repo, and Sari see a known advancing helper in slot 0 of their first post-opener shop via the starter-aware opening-shop guard. The older ignored `AllStarterMainFlowAudit` remains supporting detail, `AxiomRetryChoiceQualitySmoke` proves Axiom's configured retry helpers all advance, and `AxiomRetryEconomySmoke` covers the production retry outlier by proving the retry shop starts with a configured helper, buys/deploys it, and then progresses through the retry fight into a full Stage 2 planning shop after the nonlethal opening loss. | Covered behaviorally | Fresh human real-window replay of every starter remains optional but not mechanically required for current blockers. |
...
- All 11 first-shop starters advanced beyond Stage 2 after the bought helper: Berebell+Repo, Bo+Berebell, Bonko+Grint, Brute+Cashmere, Cashmere+Bonko, Grint+Mortem, Korath+Morrak, Morrak+Grint, Mortem+Bonko, Repo+Sari, and Sari+Grint.
- No first-shop starter resolved the second fight and stayed at Stage 2 in this refreshed default-click replay.
...
- Axiom remains the onboarding outlier in `AllStarterMainFlowAudit` because the default-bet replay records the immediate Stage 1 retry state rather than the follow-up retry fight. The stronger tracked retry coverage now proves that state can recover mechanically: `AxiomRetryChoiceQualitySmoke` advances all five configured retry helpers, and `AxiomRetryEconomySmoke` proves the production retry shop starts with one of those helpers before the buy/deploy/retry-fight path reaches a full Stage 2 planning shop.
...
`tests/visual/AxiomRetryChoiceQualitySmoke.tscn` covers the separate retry-shop path that Axiom uses instead of the normal post-victory first shop. Its latest 2026-06-26 MCP debug run completed with `AxiomRetryChoiceQualitySmoke: PASS trials=5 advanced=5` and `errors: []`: Bonko, Grint, Sari, Morrak, and Berebell each advanced the Axiom retry fight beyond Stage 1. `AxiomRetryEconomySmoke` now also asserts the production Axiom retry shop starts with one configured helper before buying/deploying and progressing to Stage 2.
...
Current 2026-06-26 automated strategy refresh from `AllStarterMainFlowAudit` after the expanded first-slot starter-aware first-shop guard:
...
- Clean default first-card advance lines: Berebell+Repo, Bo+Berebell, Bonko+Grint, Brute+Cashmere, Cashmere+Bonko, Grint+Mortem, Korath+Morrak, Morrak+Grint, Mortem+Bonko, Repo+Sari, and Sari+Grint advance beyond Stage 2 in the current automated replay.
- Soft-fail/pressure lines: none in the refreshed default-click replay. Every non-Axiom starter that reaches first shop advances beyond Stage 2 after the first clicked helper.
- Axiom remains the only current starter that does not reach the first shop in the default-bet replay; it enters the Stage 1 retry state with 2 gold and relies on the separate Axiom retry path for recovery proof. That retry path now has its own configured slot-0 helper guard, and all five configured retry helpers advance in `AxiomRetryChoiceQualitySmoke`.
...
- Current audit interpretation: the mechanical blockers, normal first-shop default-card quality, and Axiom's retry-shop default-card quality are covered for the current sensitive starters; the live design backlog is broader choice quality and starter pacing outside the protected slot-0 paths.
