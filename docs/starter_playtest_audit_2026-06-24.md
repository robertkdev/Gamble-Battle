...
- `tests/visual/NaturalAllStarterMainFlowSmoke.tscn`: `NaturalAllStarterMainFlowSmoke: PASS starters=12 first_shop=12 retry=0 deployed=12 second_resolved=12`; added and run on 2026-06-26 with `errors: []`, validating the same all-12 Main-scene loop at production `Engine.time_scale = 1.0` without the accelerated runner's planning-timer hold. This is automated UI-path coverage, not physical manual replay.
...
- The MCP debug run completed with `NaturalAllStarterMainFlowSmoke: PASS starters=12 first_shop=12 retry=0 deployed=12 second_resolved=12` and `errors: []`.
