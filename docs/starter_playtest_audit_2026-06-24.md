...
- The previous dummy-renderer run could not capture loss/exit framebuffers, but later live editor runs did. Remaining modal risk is visual quality: the synthetic loss modal screenshot shows a large mostly blank scoreboard area, the real Axiom loss screenshot shows only the player ledger row even though the extracted label summary includes an enemy row, and the system menu overlay dims the underlying game heavily.
...
| Loss and system modals | `LossScreenSmoke`, `ExitFlowSmoke`, and real Axiom loss capture provide current framebuffer evidence. | Covered visually with caveats | Decide whether hidden enemy rows should appear in modal summaries and whether the Menu button should be hidden behind defeat overlay. |
...
15. Hide or disable the top-right system Menu button while the defeat overlay is active, or make it visually subordinate to the modal.
