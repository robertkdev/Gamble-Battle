...
- RoleMatrixSmoke passes all 22 units, but the fresh 2026-06-25 detail recheck found accepted lower-level `FAIL` spans inside aggregate passes. After subject-side filtering, goal-level ramp-applicability cleanup, and live counterplay scenario generation, 21 of 22 reports still contain 86 subject-side accepted spans, with zero non-ramp identities carrying `goal_*_ramp_*` diagnostics.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the 2026-06-25 saved report parser found 86 subject-side accepted lower-level `FAIL` spans across 21/22 reports after filtering opponent-side aggregate labels, suppressing non-applicable goal ramp diagnostics, and adding live counterplay scenario rows. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
- The saved report JSON now records the subject-side backlog directly: 86 lower-level fail spans, split as 22 role spans, 46 approach spans, and 18 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
...
- Support/peel/cleanse/CC is still the largest bucket at 26 spans in the refreshed report diagnostics. Axiom and Totem pass support, but peel saves remain 0 in multiple spans; Axiom still has 0 cc-immunity grants and 0 cleanse applied in approach and role spans. The new live counterplay rows remove Grint from the debuff/lockdown context group, but add fresh counterplay-delta evidence rows for remaining content/scenario tuning.
...
The `support_peel_triage` column in `accepted_lower_level_fail_spans.csv` and `support_peel_triage_counts` in `rga_accepted_misses_summary.json` now regenerate these groups directly from current reports. The same export also adds `support_peel_gap_kind` and `support_peel_next_action`. Current gap-kind counts are: counterplay context absent 9, hard-peel submode absent 6, team peel-save metric absent 6, cooldown-trade quality below target 2, CC-prevention context absent 1, peel-interrupt context absent 1, and wombo CC-sync absent 1.
...
| Debuff/lockdown counterplay on Brute, Kythera, and Sari | 7 | Live `counterplay` rows now exist. Brute still has a high-tenacity effective-drop miss; Kythera and Sari still lack enough cleanse pressure, cleanse-bait, or scenario-delta response. Grint now passes the live debuff counterplay scenario and is no longer in this triage group. | Counterplay telemetry exists and the scenario harness is live; remaining rows need content/scenario tuning, different response composition, or retagging if these identities should not require cleanse/high-tenacity response pressure. |
| Volt `mage.pick_burst` with `burst,lockdown` | 2 | The new live counterplay label also surfaces Volt lockdown scenario-delta misses: cleanse scenario delta is negative and high-tenacity effective drop is 0. | Treat as newly exposed live counterplay tuning debt rather than a starter-flow blocker. |
