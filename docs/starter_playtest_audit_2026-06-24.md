...
- A targeted current Main-flow rapid-input runner now covers a same-frame burst across five rendered shop cards, five resulting bench units, five bench-to-board deploy attempts, and the next fight resolving.
...
- The previous dummy-renderer run could not capture loss/exit framebuffers, but later live editor runs did. Remaining modal risk is visual quality: the synthetic loss modal screenshot shows a large mostly blank scoreboard area, the real Axiom loss screenshot shows only the player ledger row even though the extracted label summary includes an enemy row, and the system menu overlay dims the underlying game heavily. The earlier top-right Menu-behind-defeat conflict is now behaviorally covered by `ExitFlowSmoke`.
...
| Loss and system modals | `LossScreenSmoke`, `ExitFlowSmoke`, and real Axiom loss capture provide current framebuffer evidence. `ExitFlowSmoke` now also proves the system Menu hides and cannot open while `LossOverlayLayer` is active. | Covered visually and behaviorally with caveats | Decide whether hidden enemy rows should appear in modal summaries; system Menu behind defeat is fixed mechanically but still needs future live-window screenshot confirmation. |
...
- This is still not a full manual pointer-coordinate test: the burst used rendered card signals, and the deploy path used the established direct drag lifecycle fallback. The remaining risk is hit-target spacing, readable feedback, and human drag feel in the live debug window.
...
11. Preserve the now-passing rapid rendered-card buy/deploy behavior, and verify the same pattern with real mouse coordinates in the debug window.
...
16. Decide whether the defeat modal should expose enemy ledger rows; if not, remove hidden enemy labels from modal summaries and make the displayed scoreboard clearly player-only.
