...
- Largest non-support or cross-role gap kinds are multi-target coverage below target (5), burst approach peak-share below target (4), direct attrition evidence below target (4), backline pressure below target (3), engage CC timing unproven (3), magic damage share below target (3), marksman role candidate/team-share diagnostic below target (3), movement distance below target (3), redirect threat-swap/taunt absent (3), body-block events absent (2), body-block prevented damage absent (2), marksman role subject damage-share diagnostic below target (2), marksman sustained-DPS goal damage share below target (2), ramp approach stack below target (2), Wombo goal peak-share below target (1), and marksman sustained-DPS goal ramp-stack below target (1). The old 6-row effective-health group is now split into sustain, peel, and support-role EHP buckets so tuning can target the right metric path.
...
The `support_peel_triage` column in `accepted_lower_level_fail_spans.csv` and `support_peel_triage_counts` in `rga_accepted_misses_summary.json` now regenerate these groups directly from current reports. The same export also adds `support_peel_gap_kind` and `support_peel_next_action`. Current gap-kind counts are: lockdown counterplay response below target 3, peel approach team-save proxy absent 3, support-role team-peel proxy absent 2, debuff cleanse bait rate below target 2, debuff cleanse pressure absent 2, debuff cleanse scenario delta below target 2, cooldown-trade quality below target 2, peel-carry goal save proxy absent 1, CC-prevention context absent 1, peel-interrupt context absent 1, and wombo CC-sync absent 1.
...
| Lockdown counterplay on Brute and Volt | 3 | Live `counterplay` rows now exist. Brute still has a high-tenacity effective-drop miss, while Volt has lockdown scenario-delta misses for negative cleanse delta and 0 high-tenacity effective drop. | Treat as lockdown-specific response tuning: adjust cleanse/high-tenacity response composition, lockdown thresholds, or lockdown tags if anti-CC response proof should not be required. |
