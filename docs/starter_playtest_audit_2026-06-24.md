...
- RoleMatrixSmoke passes all 22 units, but the fresh 2026-06-25 detail recheck found 130 raw accepted lower-level `FAIL` spans in console output. Role-report JSON is narrower after subject-side filtering: 21 of 22 reports still contain 105 subject-side accepted spans, with Cashmere clean only at the role-identity level.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the 2026-06-25 saved report parser found 105 subject-side accepted lower-level `FAIL` spans across 21/22 reports after filtering opponent-side aggregate labels. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
- The saved report JSON now records the subject-side backlog directly: 105 lower-level fail spans, split as 22 role spans, 45 approach spans, and 38 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 21 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports; Bonko is clean after opponent-side aggregate filtering. Cashmere is clean at role identity level while still logging `goal_pick_burst_kill_count 0.00 < 1.00`.
...
- Support/peel/cleanse/CC is still the largest bucket at 25 spans in the refreshed report diagnostics. Axiom and Totem pass support, but peel saves remain 0 in multiple spans; Axiom still has 0 cc-immunity grants and 0 cleanse applied in approach and role spans.
