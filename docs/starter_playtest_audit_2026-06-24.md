...
- `tests/visual/NaturalInputMainFlowSmoke.tscn`: `NaturalInputMainFlowSmoke: PASS starters=1 first_shop=1 retry=0 deployed=1 second_resolved=1`; added and run on 2026-06-26 with `errors: []`, validating Sari through production-clock title Start, Unit Select, starter selection, Start Game, first shop card click, strict no-fallback bench-to-board helper drag, and second-fight resolution. This is tracked synthetic input proof for one representative starter, not an all-starter manual replay.
...
- `tests/visual/NaturalInputMainFlowSmoke.tscn` was added on 2026-06-26 as a strict-input Sari representative path. It runs at production clock without planning-timer holds, requires the title/unit-select/shop click path to advance through synthetic mouse input rather than signal fallback, refuses drag-lifecycle fallback for the helper bench-to-board deploy, and skips only the nonessential opener-board reposition used by the broader accelerated smoke.
- The MCP debug run completed with `NaturalInputMainFlowSmoke: PASS starters=1 first_shop=1 retry=0 deployed=1 second_resolved=1` and `errors: []`. This strengthens the natural-input evidence for one full Main-flow loop, while `AllStarterMainFlowSmoke` and `NaturalAllStarterMainFlowSmoke` remain the all-starter behavioral gates.
