...
- Sustain/survival accounts for 6 spans, now split by evidence path: sustain approach EHP ratio on Berebell/Vykos, peel approach EHP ratio on Paisley/Totem, support-role team EHP proxy on Axiom, and support-role subject EHP diagnostic on Totem. Mobility/reposition accounts for 6 spans, brawler direct attrition for 4, and magic share for 3. `BrawlerDirectAttritionProbe.tscn` now proves the role metric can pass direct attrition through frontline-share, sustain EHP, and pressure evidence, so the Berebell/Bo/Mortem/Vykos rows are live evidence/tuning gaps rather than missing metric support.
