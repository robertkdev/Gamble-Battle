...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-26 after the Totem cooldown-efficiency diagnostic pass with `gap_kinds=9`, `accepted_spans=9`, `mapped_gap_kinds=9`, and `errors: []`. A later Mortem direct-attrition EHP cleanup reduced the regenerated manifest to 8 gap kinds and 8 accepted spans.
...
- The saved report JSON now records the subject-side backlog directly: 8 lower-level fail spans, split as 1 role span, 1 approach span, and 6 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 6 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports. Axiom, Berebell, Bonko, Brute, Cashmere, Korath, Luna, Morrak, Mortem, Nyxa, Paisley, Repo, Sari, Veyra, Volt, and Vykos are currently the units without a saved lower-level accepted miss.
...
- Lowest role pass rates: Hexeon assassin 0.33; Brute, Grint, Korath, Kythera, Repo, and Veyra tank at 0.67; Axiom support, Faeling mage, and all brawlers at 0.75.
- Role-family averages from the fresh reports: assassin 0.33, brawler 0.75, support 0.88, mage 0.95, marksman 0.89, and tank 0.67.
...
- Remaining non-support or cross-role gap kinds are marksman sustained-DPS goal damage share below target (1), assassin opening presence below target (1), team-fortification buff uptime absent (1), dive backline contact absent (1), and engage success targets below target (1). The old direct-attrition, pick-burst kill-count, 6-row effective-health, 5-row frontline/body-block fallback, 3-row Korath redirect-submode, 3-row ramp-stack, 2-row execute bonus-share, and 2-row Wombo third-path groups are now diagnostic fallback coverage or closed scenario proof instead of accepted-miss debt.
- `accepted_gap_kind_summary.csv` now rolls up all 8 accepted spans into 8 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 8 gap kinds and 8 spans, and that every gap kind maps to one or more committed validation scenes.
