...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-25 after the latest accepted-miss export refresh with `gap_kinds=23`, `accepted_spans=29`, `mapped_gap_kinds=23`, and `errors: []`.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the 2026-06-25 saved report parser found 29 subject-side accepted lower-level `FAIL` spans across 17/22 reports after filtering opponent-side aggregate labels, suppressing non-applicable goal ramp, soft-peel hard-peel, and non-attrition brawler direct-attrition diagnostics, adding live counterplay scenario rows, demoting AoE low-median rows to diagnostics when alternate max-target or AoE-DPS evidence satisfies the approach, recognizing burst pressure for direct attrition, demoting low burst peak-share rows when peak DPS satisfies the approach, demoting auxiliary marksman role share rows, demoting alternate marksman side-backline rows when subject ranged/time-on-target evidence proves positioning, demoting low mage share rows when peak-over-mean evidence proves periodicity, demoting low reposition subspans when total path distance satisfies the k-of-n approach, demoting low/missing first-CC engage subspans when displacement plus first-action evidence satisfies the k-of-n approach, demoting low team-save fallback spans when direct peel/support evidence proves the aggregate, demoting low EHP fallback spans when effective HPS, ally-protection, or support-event evidence proves the aggregate, and demoting low counterplay response rows when direct debuff/lockdown evidence proves the aggregate. Latest accepted-miss export refresh reports 29 spans across 23 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` passes. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
Latest refresh after the counterplay response fallback diagnostic cleanup: a new MCP run wrote fresh JSON reports for all 22 current units on 2026-06-25 local time. Direct console verification found every current role, primary-goal, and approach verdict at `PASS`, and `get_debug_output()` returned `errors: []`. `Export-AcceptedMisses.ps1` regenerated the artifact with `reports=22`, `spans=29`, `ramp_spans=3`, and `non_ramp_goal_ramp=0`; `AcceptedMissGuardCoverageSmoke.tscn` then printed `gap_kinds=23 accepted_spans=29 mapped_gap_kinds=23` and `PASS` with `errors: []`.
...
- The saved report JSON now records the subject-side backlog directly: 29 lower-level fail spans, split as 2 role spans, 9 approach spans, and 18 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 17 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports. Axiom, Berebell, Bonko, Nyxa, and Vykos are currently the units without a saved lower-level accepted miss.
...
- Largest non-support or cross-role gap kinds are body-block events absent (2), body-block prevented damage absent (2), execute bonus share absent (2), marksman sustained-DPS goal damage share below target (2), ramp approach stack below target (2), direct attrition evidence below target (1), redirect explicit threat-swap absent (1), redirect target-swap absent (1), redirect taunt absent (1), Wombo goal peak-share below target (1), and marksman sustained-DPS goal ramp-stack below target (1). The old 6-row effective-health group is now diagnostic fallback coverage instead of accepted-miss debt.
- `accepted_gap_kind_summary.csv` now rolls up all 29 accepted spans into 23 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 23 gap kinds and 29 spans, and that every gap kind maps to one or more committed guard scenes.
