...
- Current seeded random later-shop checkpoint on 2026-06-26: `RandomLaterShopProgressionSmoke: OK samples=3 target_stage=5 audit_gold_added=28`; the MCP debug read returned `errors: []`. The smoke seeds the live Shop RNG, uses Bonko plus the normal protected first-shop opener, leaves later shop rows natural, buys up to two units per shop with a simple board-width/frontline-first heuristic, deploys through the shared drag path, and proves all three sampled natural-offer lines reach Stage 5. It is still audit-gold assisted and Bonko-only, so it narrows random later-offer risk without proving natural economy or all-starter long-form balance.
...
| Planned mid-run progression line | `MidRunProgressionSmoke` drives Bonko through the real Main scene opener, forces three later cost-1 offer rows, buys Morrak/Grint, Sari/Brute, and Berebell/Bo through rendered shop cards, deploys each bench unit through the shared drag path, and proves the planned line advances Stage 2 -> 3 -> 4 -> 5 with `errors: []`. | Covered for one audit-assisted deterministic board-width line | Broader later-choice quality still needs random-shop and multi-starter coverage. Do not treat this as proof that all later shops, all starters, or natural human-speed full runs are balanced. |
| Seeded random later-shop progression | `RandomLaterShopProgressionSmoke` seeds the live Shop RNG for three Bonko samples, keeps the protected first-shop opener, then leaves later shop rows natural. It uses audit gold to avoid economy starvation, buys up to two visible shop cards per shop with a simple board-width/frontline-first heuristic, deploys through the same bench-to-board drag path, and proves every sampled line reaches Stage 5 with `errors: []`. | Covered for three Bonko natural-offer samples under audit gold | Still not natural economy proof, all-starter long-form balance, or physical manual replay. Broaden by starter, economy realism, and seed count if later-shop quality becomes the active balance target. |
...
- Seeded random later-shop evidence: `RandomLaterShopProgressionSmoke` adds three Bonko samples where later shop rows are natural from seeded live Shop RNG, the picker buys visible cards by a simple board-width/frontline-first heuristic, and every sample reaches Stage 5. This narrows the random-offer quality concern, but only under audit gold and for one starter.
- Current audit interpretation: the mechanical blockers, normal first-shop default-card quality, known first-shop trap-card exposure, fallback retry-shop default-card quality, one deterministic mid-run planned line, and three seeded Bonko natural-offer later-shop samples are covered. The live design backlog is broader all-starter later-choice balance, natural-economy pacing, and fresh real-window screenshot evidence outside the protected opening-shop path.
