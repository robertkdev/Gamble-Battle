...
- `tests/visual/FirstShopChoiceQualitySmoke.tscn`: `FirstShopChoiceQualitySmoke: PASS starters=7 trials=35 advanced=24`; rerun on 2026-06-26 with `errors: []`, validating Bo, Bonko, Cashmere, Korath, Mortem, Repo, and Sari each retain at least one deterministic first-shop helper path that advances beyond Stage 2 while still exposing bad pure-support or redundant-defense choices. The advanced count is run telemetry, not the acceptance threshold.
...
- `outputs/audit_playtest/AllStarterLiveCapture.tscn`: `AllStarterMainFlowAudit: OK starters=12`; rerun on 2026-06-26 with `errors: []` after the opener tuning and expanded first-slot starter-aware first-shop guard. The refreshed default replay reached first shop for all 12 starters, deployed all 12 helpers, resolved all 12 second fights, and recorded no starter failures; the legacy MCP framebuffer was unavailable, so it wrote 72 skipped-capture records and 0 PNGs.
...
- The current committed validation now includes an automated Main-flow replay for all 12 current starters after the cost-tier, stage/reward, and opener tuning changes, but not a fresh human real-window/screenshot replay of every starter. The refreshed ignored live-capture runner completed behaviorally for all 12 starters, but the legacy MCP framebuffer was unavailable and all 72 capture records were skipped instead of saved as PNGs.
...
- Summary counts: `starter_count=12`, `first_shop_count=12`, `first_retry_count=0`, `deploy_success_count=12`, `second_resolved_count=12`, `starter_failures=[]`, and no shop errors. Capture counts were `saved=0`, `skipped=72` because the framebuffer was unavailable.
...
- This remains automated behavioral coverage. It does not close the separate live-window screenshot and human drag-feel gaps. Bo, Bonko, Cashmere, Korath, Mortem, Repo, and Sari still have visible first-shop choice-quality differences, but the tracked opening-shop safeguard now guarantees each of them sees a known advancing helper in the first visible/default-click slot of the first post-opener shop.
...
- Bo advanced with Berebell, Grint, and Brute; Bo did not advance with Cashmere.
...
- Summary: `trial_count=35`, `advanced_count=24`, and every targeted starter had at least one first-shop helper choice that advanced beyond Stage 2.
...
- Pure support as the first helper is especially risky: the ignored runner failed Axiom for Bo, Bonko, and Cashmere, while the latest committed guard failed Axiom for Bonko and Repo.
...
- Current audit recommendation: keep the slot-0 starter-aware guard for the current first-shop-sensitive starters, and treat broader optional choices as tuning work rather than mechanical blockers.
...
Current 2026-06-26 automated strategy refresh from `AllStarterLiveCapture` after opener tuning and the expanded first-slot starter-aware first-shop guard:
...
- Current first-shop choice sensitivity audit: Bo, Bonko, Cashmere, Korath, Mortem, Repo, and Sari each have at least one first-shop helper that advances beyond Stage 2, but 11 of 35 slot trials still fail to advance. Axiom as the first helper failed in both tested committed pairings, and redundant defensive tank helpers remain risky.
- Current starter-aware first-shop sampling guard: over 240 seeded `ShopRoller.roll_opening_for_starter(...)` shops, every guarded starter saw a known advancing helper in slot 0 with a `first_good` rate of 1.000. This closes random-offer dead ends and protects the default first click for those starters.
- Tracked first-shop matrix detail now lives in `docs/starter_first_shop_choice_matrix_2026-06-26.md`: latest committed guard telemetry is 24 of 35 advancing forced helper choices plus the starter-aware slot-0 sampling read. Repo's guard list remains conservative because Sari is the only proven advancing helper in the committed matrix.
- Current audit interpretation: the mechanical blockers, normal first-shop default-card quality, and fallback retry-shop default-card quality are covered for the current sensitive starters; the live design backlog is broader choice quality, starter pacing, and fresh real-window screenshot evidence outside the protected slot-0 paths.
