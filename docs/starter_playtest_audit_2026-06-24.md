...
- Current seeded random later-shop checkpoint on 2026-06-26: `RandomLaterShopProgressionSmoke: OK samples=6 reached=6 starters=["axiom","bonko","cashmere","grint","repo","sari"] target_stage=5 audit_gold_added=60`; the MCP debug read returned `errors: []`. The smoke seeds the live Shop RNG with one deterministic seed per representative starter, keeps the normal protected first-shop opener, leaves later shop rows natural, buys up to two units per shop with a simple board-width/frontline-first heuristic, deploys through the shared drag path, and proves all six sampled natural-offer lines reach Stage 5. It is still audit-gold assisted and representative multi-starter coverage, so it narrows random later-offer risk without proving natural economy or exhaustive all-12 long-form balance.
...
- Seeded random later-shop evidence: `RandomLaterShopProgressionSmoke` adds six representative starter samples (`axiom`, `bonko`, `cashmere`, `grint`, `repo`, `sari`) where later shop rows are natural from seeded live Shop RNG, the picker buys visible cards by a simple board-width/frontline-first heuristic, and every sample reaches Stage 5. This narrows the random-offer quality concern, but only under audit gold and for representative starters.
- Current audit interpretation: the mechanical blockers, normal first-shop default-card quality, known first-shop trap-card exposure, fallback retry-shop default-card quality, one deterministic mid-run planned line, and six representative seeded natural-offer later-shop samples are covered. The live design backlog is exhaustive all-12 starter later-choice balance, natural-economy pacing, and fresh real-window screenshot evidence outside the protected opening-shop path.
