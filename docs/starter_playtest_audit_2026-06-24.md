...
- `tests/visual/BarItemsTraitsCapture.tscn`: `BarItemsTraitsCapture: OK planning_visible_bars=0 combat_visible_bars=6 filled_item_cards=8 visible_trait_icons=3 tooltip_edge_skips=2`; rerun on 2026-06-26 with `errors: []`, validating forced filled item slots, rendered trait icons, item and trait hover tooltip show/clear paths, hidden planning-grid ProgressBars, and visible combat actor ProgressBars. Fresh PNG capture was skipped under the MCP dummy framebuffer, and the tooltip edge-clamp subchecks were skipped only because the dummy viewport was smaller than the tooltips.
...
| Item, trait, and bar UI | `BarItemsTraitsCapture` forces a filled item inventory and trait state, then validates eight filled item cards, three visible trait icons, item and trait tooltip hover show/clear behavior, zero visible planning-grid ProgressBars, and six visible combat actor ProgressBars. Its MCP dummy-renderer rerun skips PNG capture before `ViewportTexture.get_image()` and skips tooltip edge clamps only when the 64x64 dummy viewport is smaller than the tooltip. | Covered behaviorally under MCP; visual screenshot proof remains live-renderer dependent | Optional live-renderer screenshot refresh if item, trait, or combat-bar polish changes. Preserve the planning-vs-combat bar visibility split and tooltip hover paths. |
