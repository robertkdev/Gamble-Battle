...
- Marksman positioning and damage-share misses account for 10 spans. The exporter now separates five auxiliary role damage-share diagnostics on Nyxa/Sari/Teller from two sustained-DPS goal damage-share gaps on Sari/Teller; the remaining three are backline-pressure role spans. This keeps "low team share" from implying the role metric failed when sustained leadership plus ranged/backline presence already proved marksman identity.
...
- Largest non-support or cross-role gap kinds are marksman role damage-share diagnostic below target (5), multi-target coverage below target (5), body-block evidence absent (4), burst approach peak-share below target (4), direct attrition evidence below target (4), backline pressure below target (3), engage CC timing unproven (3), magic damage share below target (3), movement distance below target (3), redirect threat-swap/taunt absent (3), marksman sustained-DPS goal damage share below target (2), ramp approach stack below target (2), Wombo goal peak-share below target (1), and marksman sustained-DPS goal ramp-stack below target (1). The old 6-row effective-health group is now split into sustain, peel, and support-role EHP buckets so tuning can target the right metric path.
...
The `support_peel_triage` column in `accepted_lower_level_fail_spans.csv` and `support_peel_triage_counts` in `rga_accepted_misses_summary.json` now regenerate these groups directly from current reports. The same export also adds `support_peel_gap_kind` and `support_peel_next_action`. Current gap-kind counts are: debuff counterplay response below target 6, lockdown counterplay response below target 3, peel approach team-save proxy absent 3, support-role team-peel proxy absent 2, cooldown-trade quality below target 2, peel-carry goal save proxy absent 1, CC-prevention context absent 1, peel-interrupt context absent 1, and wombo CC-sync absent 1.
...
| Debuff counterplay on Kythera and Sari | 6 | Live `counterplay` rows now exist, but Kythera and Sari still lack enough cleanse pressure, cleanse-bait, or scenario-delta response. Grint now passes the live debuff counterplay scenario and is no longer in this triage group. | Counterplay telemetry exists and the scenario harness is live; remaining rows need cleanse-response composition/content tuning or retagging if these identities should not require cleanse response pressure. |
