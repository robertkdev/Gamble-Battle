...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-26 after the execute bonus-share diagnostic cleanup with `gap_kinds=16`, `accepted_spans=19`, `mapped_gap_kinds=16`, and `errors: []`.
...
Latest refresh after the execute bonus-share diagnostic cleanup: a new MCP run wrote fresh JSON reports for all 22 current units on 2026-06-26. Direct console verification found every current role, primary-goal, and approach verdict at `PASS`, and `get_debug_output()` returned `errors: []`. `Export-AcceptedMisses.ps1` regenerated the artifact with `reports=22`, `spans=19`, `ramp_spans=3`, and `non_ramp_goal_ramp=0`; `AcceptedMissGuardCoverageSmoke.tscn` then printed `gap_kinds=16 accepted_spans=19 mapped_gap_kinds=16` and `PASS` with `errors: []`.
...
- The saved report JSON now records the subject-side backlog directly: 19 lower-level fail spans, split as 2 role spans, 4 approach spans, and 13 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 13 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports. Axiom, Berebell, Bonko, Brute, Korath, Morrak, Nyxa, Repo, and Vykos are currently the units without a saved lower-level accepted miss.
...
- Largest non-support or cross-role gap kinds are marksman sustained-DPS goal damage share below target (2), pick-burst kill count absent (2), ramp approach stack below target (2), direct attrition evidence below target (1), Wombo goal peak-share below target (1), and marksman sustained-DPS goal ramp-stack below target (1). The old 6-row effective-health group, 5-row frontline/body-block fallback group, 3-row Korath redirect-submode group, and 2-row execute bonus-share group are now diagnostic fallback coverage instead of accepted-miss debt.
- `accepted_gap_kind_summary.csv` now rolls up all 19 accepted spans into 16 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 16 gap kinds and 19 spans, and that every gap kind maps to one or more committed validation scenes.
