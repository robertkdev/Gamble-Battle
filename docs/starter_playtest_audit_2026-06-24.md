...
- Largest non-support or cross-role gap kinds are multi-target coverage below target (5), burst approach peak-share below target (4), direct attrition evidence below target (4), backline pressure below target (3), engage CC timing unproven (3), magic damage share below target (3), marksman role candidate/team-share diagnostic below target (3), movement distance below target (3), body-block events absent (2), body-block prevented damage absent (2), marksman role subject damage-share diagnostic below target (2), marksman sustained-DPS goal damage share below target (2), ramp approach stack below target (2), redirect explicit threat-swap absent (1), redirect target-swap absent (1), redirect taunt absent (1), Wombo goal peak-share below target (1), and marksman sustained-DPS goal ramp-stack below target (1). The old 6-row effective-health group is now split into sustain, peel, and support-role EHP buckets so tuning can target the right metric path.
...
- `tests/rga_testing/validation/CounterplayContextTriageSmoke.tscn` guards the synthetic counterplay interpretation, and `RoleMatrixSmoke.tscn` now exercises live counterplay response rows. Grint's live row now passes, while Brute/Kythera/Sari/Volt still leave accepted counterplay spans for future content/scenario tuning.
