...
- Ramp-state misses now account for 3 spans, all on ramp-tagged identities: two approach-level stack misses on Sari/Veyra and one Sari sustained-DPS goal ramp-stack miss. `MarksmanSustainedDpsGoalProbe.tscn` now proves Sari's goal-level ramp-stack span can pass from direct `ramp_state_changed` telemetry and preserves an aggregate-pass low-ramp control with `goal_marksman_sustained_dps_ramp_stack_max` below target. Prior non-ramp attrition ramp noise is gone, and the export classifier also avoids treating `sustained_dps` as sustain/survival.
