...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-25 after the latest accepted-miss export refresh with `gap_kinds=20`, `accepted_spans=24`, `mapped_gap_kinds=20`, and `errors: []`.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the 2026-06-25 saved report parser found 24 subject-side accepted lower-level `FAIL` spans across 15/22 reports after filtering opponent-side aggregate labels, suppressing non-applicable goal ramp, soft-peel hard-peel, and non-attrition brawler direct-attrition diagnostics, adding live counterplay scenario rows, demoting AoE low-median rows to diagnostics when alternate max-target or AoE-DPS evidence satisfies the approach, recognizing burst pressure for direct attrition, demoting low burst peak-share rows when peak DPS satisfies the approach, demoting auxiliary marksman role share rows, demoting alternate marksman side-backline rows when subject ranged/time-on-target evidence proves positioning, demoting low mage share rows when peak-over-mean evidence proves periodicity, demoting low reposition subspans when total path distance satisfies the k-of-n approach, demoting low/missing first-CC engage subspans when displacement plus first-action evidence satisfies the k-of-n approach, demoting low team-save fallback spans when direct peel/support evidence proves the aggregate, demoting low EHP fallback spans when effective HPS, ally-protection, or support-event evidence proves the aggregate, demoting low counterplay response rows when direct debuff/lockdown evidence proves the aggregate, and demoting low frontline/body-block rows when prevention plus frontline presence proves the aggregate. Latest accepted-miss export refresh reports 24 spans across 20 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` passes. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
Latest refresh after the frontline fallback diagnostic cleanup: a new MCP run wrote fresh JSON reports for all 22 current units on 2026-06-25 local time. Direct console verification found every current role, primary-goal, and approach verdict at `PASS`, and `get_debug_output()` returned `errors: []`. `Export-AcceptedMisses.ps1` regenerated the artifact with `reports=22`, `spans=24`, `ramp_spans=3`, and `non_ramp_goal_ramp=0`; `AcceptedMissGuardCoverageSmoke.tscn` then printed `gap_kinds=20 accepted_spans=24 mapped_gap_kinds=20` and `PASS` with `errors: []`.
...
- The saved report JSON now records the subject-side backlog directly: 24 lower-level fail spans, split as 2 role spans, 9 approach spans, and 13 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 15 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports. Axiom, Berebell, Bonko, Brute, Nyxa, Repo, and Vykos are currently the units without a saved lower-level accepted miss.
...
- Tank/frontline semantics have 4 remaining redirect/engage misses. Brute/Repo body-block and Brute damage-share rows are now diagnostic when prevention plus frontline-position evidence proves `tank.frontline_absorb`. `FrontlineBodyBlockGoalProbe.tscn` still proves real redirect-kernel telemetry feeds Brute's body-block event and prevented-damage goal spans, while event-only, damage-only, and weak-prevention controls fail. `BruteFrontlineShareGoalProbe.tscn` proves Brute's frontline damage-taken-share span can pass from direct incoming-share evidence and guards the low-share/no-body-block diagnostic fallback. Korath's remaining redirect misses are split into target-swap, explicit threat-swap, and taunt-command evidence gaps. `KorathRedirectAcceptedMissProbe.tscn` proves those spans can pass from direct redirect evidence, while body-block aggregate evidence preserves the current missing-submode accepted-miss shape. `GrintEngageSuccessGoalProbe.tscn` proves Grint's initiate-fight success-target span can pass from direct multi-target engage evidence, while distance plus first-action evidence preserves the current low-success-target accepted-miss shape.
- Burst/execute/kill accounts for 5 primary spans. Cashmere and Volt pass pick-burst while kill count is 0; Luna and Paisley pass wombo/mage identity despite low CC-sync, peak-share, or magic-share evidence; Hexeon passes aggregate assassin identity while backline-fraction and execute subspans still miss. `AssassinOpeningRoleProbe.tscn` now proves real `BacklineAccessKernel` position telemetry can feed Hexeon's side-level `a_first_frac` and subject-level `subject_first_backline_frac` assassin role spans, while a late/access-losing control fails; the Hexeon opening-presence row is live opening-access scenario debt rather than missing metric support. `BurstWindowKernelProbe.tscn` now proves concentrated combat-pattern hits record peak 1s share, peak DPS, overkill, and counterplay-window telemetry and pass `approach_burst`; it also proves low peak-share rows remain diagnostic when peak DPS already proves the approach, so the old Berebell/Hexeon/Mortem/Vykos burst peak-share accepted-miss group is closed. `PickBurstKillGoalProbe.tscn` now proves lethal combat-pattern telemetry feeds `goal_pick_burst_kill_count` for the real Cashmere `mage.pick_burst` goal, while a nonlethal aggregate pass keeps the kill-count span failing; the Cashmere/Volt kill-count rows are live kill-securing/scenario debt rather than missing metric support. `ExecuteBonusApproachProbe.tscn` now proves real Hexeon/Morrak `execute` approach rows can consume direct execute-bonus events, bonus damage share, low-HP kill conversion, and overkill guardrails; its aggregate-pass controls preserve the current accepted-miss shape where Hexeon/Morrak still pass `approach_execute` while `subject_execute_bonus_damage_share` is below target. `WomboComboGoalProbe.tscn` now proves real Luna/Paisley `mage.wombo_combo_burst` goal rows can consume direct combat-pattern peak-share and multi-target evidence plus control-mobility CC event evidence. Its aggregate-pass controls preserve the current accepted-miss shapes: Luna can pass through peak+targets while CC-sync proxy fails, and Paisley can pass through targets+CC while peak-share is below target.
...
- Largest non-support or cross-role gap kinds are execute bonus share absent (2), marksman sustained-DPS goal damage share below target (2), pick-burst kill count absent (2), ramp approach stack below target (2), direct attrition evidence below target (1), redirect explicit threat-swap absent (1), redirect target-swap absent (1), redirect taunt absent (1), Wombo goal peak-share below target (1), and marksman sustained-DPS goal ramp-stack below target (1). The old 6-row effective-health group and the 5-row frontline/body-block fallback group are now diagnostic fallback coverage instead of accepted-miss debt.
- `accepted_gap_kind_summary.csv` now rolls up all 24 accepted spans into 20 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 20 gap kinds and 24 spans, and that every gap kind maps to one or more committed guard scenes.

Support/peel/cleanse/CC bucket triage from the current saved report JSON:

The `support_peel_triage` column in `accepted_lower_level_fail_spans.csv` and `support_peel_triage_counts` in `rga_accepted_misses_summary.json` now regenerate these groups directly from current reports. The same export also adds `support_peel_gap_kind` and `support_peel_next_action`. Current gap-kind counts are: CC-immunity approach cooldown trade below target 1, peel-carry goal cooldown trade below target 1, peel-carry goal save proxy absent 1, CC-prevention context absent 1, peel-interrupt context absent 1, and wombo CC-sync absent 1.
