...
- The current committed validation now includes an automated Main-flow replay for all 12 current starters after the cost-tier, stage/reward, and opener tuning changes, plus a fresh Godot-AI live-editor PNG set for every starter and capture milestone. Natural-speed spot checks now include Sari through title Start, Unit Select, first planning, bench-to-board deployment, and opener resolving; Repo through title Start, Unit Select, guarded first shop, Sari purchase feedback, and the same-stage Round 2 defeat retry when the helper remains benched; and Axiom through title Start, support-starter selection, first deploy, production Round 2 shop, Bo purchase, and helper deploy. This is still not a natural human-speed replay of every starter: the all-starter runner is automated/accelerated, and some later planning captures show the harness-held timer rather than normal player timing.
...
| All current starters through first Main-flow loop | `AllStarterMainFlowSmoke` is now committed validation for all 12 current starters reaching first shop, buying/deploying a helper, and resolving the second fight through the real Main scene with zero opener retries. The Chapter 1 Round 1 Beegle is tuned as a short-range tutorial target only for that opener (`max_hp=120`, `attack_damage=50`, `attack_range=1`), while later Beegles keep their normal profile. The smoke still asserts Bo, Bonko, Cashmere, Korath, Mortem, Repo, and Sari see a known advancing helper in slot 0 of their first post-opener shop via the starter-aware opening-shop guard, while `FirstShopOfferQualitySamplingSmoke` asserts configured known-bad helpers are absent from sampled guarded opening shops. The ignored `AllStarterLiveCapture` replay now adds a clean Godot-AI live-editor PNG set: 72 saved captures, 0 skipped, `display_server=Windows`, `rendering_driver=opengl3`, and matching manifest counts for all 12 starters. Natural-speed spot checks now cover Sari title-to-deploy/opener, Repo title-to-guarded-shop/purchase/retry behavior, and Axiom support-starter title-to-production-shop/helper-deploy behavior. `AxiomRetryChoiceQualitySmoke` and `AxiomRetryEconomySmoke` now force the hard opener internally to keep fallback retry recovery covered. | Covered behaviorally and visually through automated live-editor captures; natural spot checks cover one clean deploy starter, one weak-starter shop/retry path, and one support-starter production helper-deploy path | Fresh natural human-speed replay of every starter remains optional but not mechanically required for current blockers; preserve the caveat that the all-starter PNG set is automated/accelerated and some timer labels reflect harness-held time. |
