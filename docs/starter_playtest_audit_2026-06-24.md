...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-26 after the Wombo third-path diagnostic cleanup with `gap_kinds=12`, `accepted_spans=14`, `mapped_gap_kinds=12`, and `errors: []`.
...
- RoleMatrixSmoke passes all 22 units, but the fresh detail recheck found accepted lower-level `FAIL` spans inside aggregate passes. After subject-side filtering, goal-level ramp-applicability cleanup, live counterplay scenario generation, hard-peel applicability filtering, AoE median diagnostic cleanup, brawler direct-attrition cleanup, burst peak-share diagnostic cleanup, marksman auxiliary share diagnostic cleanup, marksman alternate ranged-positioning diagnostic cleanup, mage alternate periodicity diagnostic cleanup, reposition alternate path-distance cleanup, engage alternate timing cleanup, support/peel team-save fallback cleanup, EHP fallback diagnostic cleanup, counterplay response fallback cleanup, frontline fallback cleanup, Korath redirect submode diagnostic cleanup, execute bonus-share diagnostic cleanup, ramp stack diagnostic cleanup, and Wombo third-path diagnostic cleanup, 10 of 22 reports still contain 14 subject-side accepted spans, with zero non-ramp identities carrying `goal_*_ramp_*` diagnostics and zero ramp-stack accepted spans. The latest export refresh reports 14 spans across 12 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` still maps every gap kind to committed guard scenes.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the saved report parser now finds 14 subject-side accepted lower-level `FAIL` spans across 10/22 reports after filtering opponent-side aggregate labels, suppressing non-applicable goal ramp, soft-peel hard-peel, and non-attrition brawler direct-attrition diagnostics, adding live counterplay scenario rows, demoting AoE low-median rows to diagnostics when alternate max-target or AoE-DPS evidence satisfies the approach, recognizing burst pressure for direct attrition, demoting low burst peak-share rows when peak DPS satisfies the approach, demoting auxiliary marksman role share rows, demoting alternate marksman side-backline rows when subject ranged/time-on-target evidence proves positioning, demoting low mage share rows when peak-over-mean evidence proves periodicity, demoting low reposition subspans when total path distance satisfies the k-of-n approach, demoting low/missing first-CC engage subspans when displacement plus first-action evidence satisfies the k-of-n approach, demoting low team-save fallback spans when direct peel/support evidence proves the aggregate, demoting low EHP fallback spans when effective HPS, ally-protection, or support-event evidence proves the aggregate, demoting low counterplay response rows when direct debuff/lockdown evidence proves the aggregate, demoting low frontline/body-block rows when prevention plus frontline presence proves the aggregate, demoting missing redirect submodes when body-block evidence proves `approach_redirect`, demoting low execute bonus-share rows when direct execute bonus damage plus low-HP kill conversion proves `approach_execute`, demoting low ramp-stack rows when direct ramp events plus peak/window evidence prove the aggregate, and demoting Wombo's missing third evidence path when the other two direct Wombo paths satisfy the 2-of-3 goal aggregate. Latest accepted-miss export refresh reports 14 spans across 12 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` passes. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
Latest refresh after the Wombo third-path diagnostic cleanup: a new MCP run wrote fresh JSON reports for all 22 current units on 2026-06-26. Direct console verification found every current role, primary-goal, and approach verdict at `PASS`, and `get_debug_output()` returned `errors: []`. `Export-AcceptedMisses.ps1` regenerated the artifact with `reports=22`, `spans=14`, `ramp_spans=0`, and `non_ramp_goal_ramp=0`; `AcceptedMissGuardCoverageSmoke.tscn` then printed `gap_kinds=12 accepted_spans=14 mapped_gap_kinds=12` and `PASS` with `errors: []`.

Follow-up pick-burst scenario-cap validation on 2026-06-26 keeps `burst` preferred for `mage.pick_burst` identities when the smoke label set is capped. `PickBurstScenarioLabelSmoke.tscn` prints Volt-style labels as `["neutral", "counterplay", "burst"]` while preserving the prior generic defensive preference for non-pick-burst rows. A fresh `RoleMatrixSmoke.tscn` rerun again completed with `RoleMatrixSmoke: PASS (22 units)` and `errors: []`; Volt now exercises `neutral`, `counterplay`, and `burst` scenarios in the all-unit smoke. `Export-AcceptedMisses.ps1` still reports `reports=22`, `spans=14`, `ramp_spans=0`, and `non_ramp_goal_ramp=0`, so the Cashmere/Volt pick-burst rows remain kill-securing/scenario debt, not missing burst-context coverage.

Follow-up mage burst scenario-pack validation on 2026-06-26 adds a mage `burst`/`pick_burst_window` pack so `mage.pick_burst` rows no longer fall back to generic mage map params when the all-unit smoke requests `burst`. `PickBurstScenarioLabelSmoke.tscn` now also proves mage `burst` resolves to the back-lane `pick_burst_window` pack. A fresh `RoleMatrixSmoke.tscn` still passed all 22 units with `errors: []`; Cashmere and Volt still record `goal_pick_burst_kill_count: 0.00`, and `Export-AcceptedMisses.ps1` still reports `reports=22`, `spans=14`, `ramp_spans=0`, and `non_ramp_goal_ramp=0`, so the pick-burst rows are now live kill-securing/scenario debt rather than missing metric support or missing burst scenario-pack support.
...
Follow-up support carry-threat scenario-pack validation on 2026-06-26 adds a support `threat`/`carry_threat_window` pack for `support.peel_carry` audit contexts. `SupportCarryThreatScenarioPackSmoke.tscn` proves Totem-style labels keep `neutral`, `peel`, and `threat`, while Axiom-style support amplification and Veyra-style non-support CC-immunity rows do not request `threat`. A fresh `RoleMatrixSmoke.tscn` still passed all 22 units with `errors: []`; Totem's full run now includes `threat` but still preserves the five accepted lower-level misses for peel saves, interrupt events, CC prevention, and cooldown-trade efficiency. `Export-AcceptedMisses.ps1` still reports `reports=22`, `spans=14`, `ramp_spans=0`, and `non_ramp_goal_ramp=0`, so Totem's support/peel/CC rows are now live save/interrupt/prevention/cooldown-efficiency debt rather than missing metric support or missing carry-threat scenario-pack support.

Follow-up tank engage scenario-pack validation on 2026-06-26 adds a tank `engage`/`engage_window` pack for `tank.initiate_fight` and engage-tagged tank audit contexts. `TankEngageScenarioPackSmoke.tscn` proves Grint-style labels keep `neutral`, `counterplay`, `engage`, and `burst`, Brute-style engage labels keep `engage`, `counterplay`, and `peel`, and non-engage fortification tanks do not request `engage`. A fresh `RoleMatrixSmoke.tscn` still passed all 22 units with `errors: []`; Grint's full run now includes `engage` and still preserves `goal_initiate_fight_engage_success_targets: 0.00`, so the remaining Grint row is live multi-target engage-success debt rather than missing metric support or missing tank engage scenario-pack support.
...
- The saved report JSON now records the subject-side backlog directly: 14 lower-level fail spans, split as 2 role spans, 2 approach spans, and 10 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 10 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports. Axiom, Berebell, Bonko, Brute, Korath, Luna, Morrak, Nyxa, Paisley, Repo, Veyra, and Vykos are currently the units without a saved lower-level accepted miss.
...
- Lowest role pass rates: Hexeon assassin 0.33; Bo and Mortem brawler at 0.50; Brute, Grint, Korath, Kythera, Repo, and Veyra tank at 0.67; Axiom support at 0.75.
- Role-family averages from the fresh reports: assassin 0.33, brawler 0.67, support 0.88, mage 1.00, marksman 0.89, and tank 0.67.
...
- The old 6-span sustain/survival EHP fallback bucket is closed. `EhpRatioPathProbe.tscn` now proves all four EHP ratio paths can pass from aggregate unit healing/shields, source-owned support maps, and team EHP totals, and also proves low EHP rows are diagnostic when alternate effective-HPS, ally-protection, or support-event evidence satisfies the aggregate. Brawler direct attrition accounts for 1 span. `RepositionMovementKernelProbe.tscn` now proves direct movement signals can pass `approach_reposition` through max-step, post-cast displacement, and total-path spans, and the old Berebell/Bo/Mortem reposition group is closed because low max-step/post-cast rows are diagnostic when total path distance satisfies the k-of-n approach. `SkirmishDiveBacklineGoalProbe.tscn` now proves hit attribution can pass Bo's `brawler.skirmish_dive` backline-contact span through `damage_to_frontline_pct`, while an all-frontline control fails; `SkirmishDiveScenarioPackSmoke.tscn` proves the all-unit `counter` label now gives Bo a proper brawler `dive_window` context, but the current RoleMatrix run still records `goal_skirmish_dive_backline_contact_proxy: 0.00` for Bo, so the dive-contact row is live backline-access scenario debt rather than missing metric support or missing brawler dive-map support. `BrawlerDirectAttritionProbe.tscn` now proves the role metric can pass direct attrition through frontline-share, sustain EHP, and pressure evidence including burst pressure; the compiler only saves direct-attrition diagnostics for attrition-DPS brawlers. Berebell and Vykos now satisfy direct attrition through burst pressure plus sustain, Bo's non-attrition brawler row is suppressed, and Mortem remains live direct-sustain attrition debt rather than missing metric support. `MagePeriodicityKernelProbe.tscn` now proves magic hit components can pass `role_mage_identity` through top-window magic share and peak-over-mean evidence, and the old Luna/Paisley/Volt magic-share group is closed because low share is diagnostic when alternate peak-over-mean evidence proves mage periodicity.
- The old residual `other` bucket is now explicit: assassin opening presence has 1 span and team-fortification buff uptime has 1 span. `EngageCcTimingKernelProbe.tscn` now proves direct control-mobility signals can pass `approach_engage` through early displacement, first action, and first-CC timing, and the old Brute/Grint/Korath engage timing group is closed because low or missing first-CC subspans are diagnostic when displacement plus first-action evidence satisfies the k-of-n engage approach. `TeamFortificationBuffGoalProbe.tscn` now proves source-owned `buff_applied` telemetry feeds Kythera's `tank.team_fortification` ally-buff goal span, while an aggregate EHP/prevention pass keeps the buff span failing when no ally buff is present; `TeamFortificationScenarioPackSmoke.tscn` proves all-unit selection now adds a fortify context without replacing existing defensive/counterplay labels. The current RoleMatrix run still records Kythera `goal_team_fortification_buff_uptime_targets: 0.00` while Veyra's same goal span passes, so Kythera's row is live kit/identity/context debt rather than missing metric or fortification scenario support.
...
- Largest non-support or cross-role gap kinds are marksman sustained-DPS goal damage share below target (2), pick-burst kill count absent (2), direct attrition evidence below target (1), assassin opening presence below target (1), team-fortification buff uptime absent (1), dive backline contact absent (1), and engage success targets below target (1). The old 6-row effective-health group, 5-row frontline/body-block fallback group, 3-row Korath redirect-submode group, 3-row ramp-stack group, 2-row execute bonus-share group, and 2-row Wombo third-path group are now diagnostic fallback coverage instead of accepted-miss debt.
- `accepted_gap_kind_summary.csv` now rolls up all 14 accepted spans into 12 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 12 gap kinds and 14 spans, and that every gap kind maps to one or more committed validation scenes.

Support/peel/cleanse/CC bucket triage from the current saved report JSON:

The `support_peel_triage` column in `accepted_lower_level_fail_spans.csv` and `support_peel_triage_counts` in `rga_accepted_misses_summary.json` now regenerate these groups directly from current reports. The same export also adds `support_peel_gap_kind` and `support_peel_next_action`. Current gap-kind counts are: CC-immunity approach cooldown trade below target 1, peel-carry goal cooldown trade below target 1, peel-carry goal save proxy absent 1, CC-prevention context absent 1, and peel-interrupt context absent 1.

| Area | Rows | Current evidence gap | Audit decision |
| --- | ---: | --- | --- |
| Totem `support.peel_carry` with `peel,cc_immunity,amp` | 5 | The all-unit RoleMatrix row still lacks goal-level team peel saves, interrupt events, `subject_cc_prevented_as_target`, and both approach-level CC-immunity and goal-level peel-carry cooldown-trade efficiency, even though it has direct shield/amp/CC-immunity evidence. The latest all-unit row now includes the `threat` carry-threat scenario context, and approach/support-role team-save fallback rows are diagnostic when direct protection proves those aggregates. | This is save/interrupt/prevention/cooldown-efficiency tuning debt in the all-unit smoke, not proof that Totem support/peel telemetry is absent or that carry-threat scenario coverage is missing. `TotemCleanseLiveProbe.tscn` remains the explicit real-ability cleanse control, `TotemPeelCarryAcceptedMissProbe.tscn` guards the metric/accepted-miss shape, and `SupportCarryThreatScenarioPackSmoke.tscn` guards the all-unit `threat` context. |
...
Current committed regression coverage was added on 2026-06-25 as `tests/visual/RapidShopPressureSmoke.tscn`. The MCP debug run completed with `RapidShopPressureSmoke: OK purchases=5 deployed_board=6` and `errors: []`. It uses the real Main scene, selects Bonko, wins the forced opener, prepares a level-1 shop with five rendered buyable cards, emits all five card presses in one burst, verifies current `SOLD` / `On bench` placeholders, verifies no shop errors, deploys all five bench units to the board, and confirms the follow-up fight resolves.
