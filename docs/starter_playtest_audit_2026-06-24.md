...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-25 after the latest accepted-miss export refresh with `gap_kinds=17`, `accepted_spans=21`, `mapped_gap_kinds=17`, and `errors: []`.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the 2026-06-25 saved report parser found 21 subject-side accepted lower-level `FAIL` spans across 14/22 reports after filtering opponent-side aggregate labels, suppressing non-applicable goal ramp, soft-peel hard-peel, and non-attrition brawler direct-attrition diagnostics, adding live counterplay scenario rows, demoting AoE low-median rows to diagnostics when alternate max-target or AoE-DPS evidence satisfies the approach, recognizing burst pressure for direct attrition, demoting low burst peak-share rows when peak DPS satisfies the approach, demoting auxiliary marksman role share rows, demoting alternate marksman side-backline rows when subject ranged/time-on-target evidence proves positioning, demoting low mage share rows when peak-over-mean evidence proves periodicity, demoting low reposition subspans when total path distance satisfies the k-of-n approach, demoting low/missing first-CC engage subspans when displacement plus first-action evidence satisfies the k-of-n approach, demoting low team-save fallback spans when direct peel/support evidence proves the aggregate, demoting low EHP fallback spans when effective HPS, ally-protection, or support-event evidence proves the aggregate, demoting low counterplay response rows when direct debuff/lockdown evidence proves the aggregate, demoting low frontline/body-block rows when prevention plus frontline presence proves the aggregate, and demoting missing redirect submodes when body-block evidence proves `approach_redirect`. Latest accepted-miss export refresh reports 21 spans across 17 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` passes. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
Latest refresh after the Korath redirect submode diagnostic cleanup: a new MCP run wrote fresh JSON reports for all 22 current units on 2026-06-25 local time. Direct console verification found every current role, primary-goal, and approach verdict at `PASS`, and `get_debug_output()` returned `errors: []`. `Export-AcceptedMisses.ps1` regenerated the artifact with `reports=22`, `spans=21`, `ramp_spans=3`, and `non_ramp_goal_ramp=0`; `AcceptedMissGuardCoverageSmoke.tscn` then printed `gap_kinds=17 accepted_spans=21 mapped_gap_kinds=17` and `PASS` with `errors: []`.
...
- The saved report JSON now records the subject-side backlog directly: 21 lower-level fail spans, split as 2 role spans, 6 approach spans, and 13 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 14 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports. Axiom, Berebell, Bonko, Brute, Korath, Nyxa, Repo, and Vykos are currently the units without a saved lower-level accepted miss.
...
- Support/peel/cleanse/CC is now the largest bucket at 6 spans, followed by burst/execute/kill at 5 and ramp-state at 3. Soft-peel identities no longer persist missing hard-peel cleanse/CC-immunity subspans unless their identity claims `support.peel_carry` or `cc_immunity`, low team-save fallback rows are now diagnostic when direct ally-protection/support evidence proves the aggregate, low EHP fallback rows are now diagnostic when effective HPS, ally-protection, or support-event evidence proves the aggregate, low counterplay response rows are now diagnostic when direct debuff/lockdown evidence proves the aggregate, low frontline/body-block rows are now diagnostic when prevention plus frontline presence proves the goal, and missing Korath redirect submodes are diagnostic when body-block evidence proves `approach_redirect`. `PeelTeamSaveProxyProbe.tscn` proves the shared team-save proxy can pass `approach_peel`, `role_support_identity`, and `support.peel_carry` from one derived peel save, and `SoftPeelTeamSaveAcceptedMissProbe.tscn` proves real Axiom/Paisley soft-peel rows can pass from team-save evidence while aggregate ally-protection controls keep low team-save fallback rows diagnostic. Totem's goal-level peel-save row remains live peel-carry scenario debt, while its approach and support-role team-save and EHP fallback rows are diagnostic. `TotemPeelCarryAcceptedMissProbe.tscn` now proves real Totem `support.peel_carry` rows can pass all support/peel consumers with team-save, EHP, CC-prevention, interrupt, and cooldown evidence, while aggregate direct-protection controls preserve the remaining goal and hard-peel lower-level miss shape. Kythera/Sari debuff and Brute/Volt lockdown response rows are closed as accepted-miss debt because direct debuff/lockdown evidence keeps the aggregates passing and the low response-pressure rows are diagnostic-only.
...
- Largest non-support or cross-role gap kinds are execute bonus share absent (2), marksman sustained-DPS goal damage share below target (2), pick-burst kill count absent (2), ramp approach stack below target (2), direct attrition evidence below target (1), Wombo goal peak-share below target (1), and marksman sustained-DPS goal ramp-stack below target (1). The old 6-row effective-health group, 5-row frontline/body-block fallback group, and 3-row Korath redirect-submode group are now diagnostic fallback coverage instead of accepted-miss debt.
- `accepted_gap_kind_summary.csv` now rolls up all 21 accepted spans into 17 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 17 gap kinds and 21 spans, and that every gap kind maps to one or more committed validation scenes.
