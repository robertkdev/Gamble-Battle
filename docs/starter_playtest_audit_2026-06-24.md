...
- `tests/visual/NaturalBuyXPVisualSmoke.tscn`: `NaturalBuyXPVisualSmoke: OK attempts=4 stage=2 gold_before=5 gold_after=1` in a live Godot-AI/OpenGL run, saving `outputs/visual_iter/natural_buy_xp_pass/01_natural_buy_xp_ready.png` and `02_natural_buy_xp_success.png`; the legacy MCP debug rerun skipped PNG capture under dummy rendering but passed with `NaturalBuyXPVisualSmoke: OK attempts=4 stage=2 gold_before=5 gold_after=1` and `errors: []`.
...
- The committed current natural Buy XP proof is `tests/visual/NaturalBuyXPVisualSmoke.tscn`: a live Godot-AI/OpenGL run retried ordinary Bonko opener outcomes until natural reward gold produced a safe 5g Round 2 shop, then clicked the visible Buy XP button and captured the UI repaint from `Gold: 5` / `Lvl 1 (0/2)` to `Gold: 1` / `Lvl 2 (2/6)`. The legacy MCP debug rerun repeated the same model path with `errors: []`, skipping only PNG capture under dummy rendering.
...
Fresh tracked visual evidence was generated on 2026-06-26 with `tests/visual/NaturalBuyXPVisualSmoke.tscn`. The smoke drives the real Main scene from Bonko selection through the forced opener and retries fresh ordinary opener outcomes until natural reward gold creates a safe Buy XP window. It does not grant audit gold or force shop offers. The live Godot-AI/OpenGL run saved screenshots under `outputs/visual_iter/natural_buy_xp_pass/`; the legacy MCP debug rerun passed the same assertions and skipped PNG capture under dummy rendering.
...
- Current tracked natural visual result: the live Godot-AI/OpenGL run reached the safe window on attempt 4, saved `01_natural_buy_xp_ready.png` with visible `Gold: 5` and `Lvl 1 (0/2)`, clicked the visible Buy XP button, then saved `02_natural_buy_xp_success.png` with visible `Gold: 1` and `Lvl 2 (2/6)`. The latest legacy MCP rerun reached the same state on attempt 4 and reported `errors: []`.
