...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-25 after the latest accepted-miss export refresh with `gap_kinds=39`, `accepted_spans=63`, `mapped_gap_kinds=39`, and `errors: []`.
...
- RoleMatrixSmoke passes all 22 units, but the fresh 2026-06-25 detail recheck found accepted lower-level `FAIL` spans inside aggregate passes. After subject-side filtering, goal-level ramp-applicability cleanup, live counterplay scenario generation, hard-peel applicability filtering, AoE median diagnostic cleanup, brawler direct-attrition cleanup, burst peak-share diagnostic cleanup, and marksman auxiliary share diagnostic cleanup, 21 of 22 reports still contain 63 subject-side accepted spans, with zero non-ramp identities carrying `goal_*_ramp_*` diagnostics. The latest export refresh reports 63 spans across 39 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` still maps every gap kind to committed guard scenes.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the 2026-06-25 saved report parser found 63 subject-side accepted lower-level `FAIL` spans across 21/22 reports after filtering opponent-side aggregate labels, suppressing non-applicable goal ramp, soft-peel hard-peel, and non-attrition brawler direct-attrition diagnostics, adding live counterplay scenario rows, demoting AoE low-median rows to diagnostics when alternate max-target or AoE-DPS evidence satisfies the approach, recognizing burst pressure for direct attrition, demoting low burst peak-share rows when peak DPS satisfies the approach, and demoting auxiliary marksman role share rows. Latest accepted-miss export refresh reports 63 spans across 39 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` passes. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
Latest refresh after the marksman auxiliary share diagnostic cleanup: a new MCP run wrote fresh JSON reports for all 22 current units on 2026-06-25 local time. Direct console verification found every current role, primary-goal, and approach verdict at `PASS`, and `get_debug_output()` returned `errors: []`. `Export-AcceptedMisses.ps1` regenerated the artifact with `reports=22`, `spans=63`, `ramp_spans=3`, and `non_ramp_goal_ramp=0`; `AcceptedMissGuardCoverageSmoke.tscn` then printed `gap_kinds=39 accepted_spans=63 mapped_gap_kinds=39` and `PASS` with `errors: []`.
...
- The saved report JSON now records the subject-side backlog directly: 63 lower-level fail spans, split as 12 role spans, 33 approach spans, and 18 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
...
- Marksman positioning and damage-share misses now account for 5 spans. The former three auxiliary candidate/team-side role share rows on Nyxa/Sari/Teller and two auxiliary subject-owned role share rows on Sari/Teller are emitted as diagnostics instead of accepted misses. The remaining marksman backlog is three backline-pressure role spans on Nyxa/Sari/Teller plus two sustained-DPS goal damage-share gaps on Sari/Teller. `MarksmanPositioningRoleProbe.tscn` now proves `role_marksman_identity` can pass through sustained DPS leadership plus direct backline/ranged presence while keeping low candidate and subject damage-share rows diagnostic-only. `MarksmanSustainedDpsGoalProbe.tscn` now proves real Sari/Teller `marksman.sustained_dps` goal rows can consume direct team damage share, range/time-on-target, survival, and Sari ramp-state evidence; its aggregate-pass controls preserve the current Sari/Teller low-damage-share rows as live output/scenario debt rather than missing metric support. This keeps "low team share" from implying the role metric failed when sustained leadership plus ranged/backline presence already proved marksman identity, and classifies the remaining Nyxa/Sari/Teller marksman rows as live positioning/output debt rather than missing metric support.
...
- Largest non-support or cross-role gap kinds are backline pressure below target (3), engage CC timing unproven (3), magic damage share below target (3), movement distance below target (3), body-block events absent (2), body-block prevented damage absent (2), execute bonus share absent (2), marksman sustained-DPS goal damage share below target (2), ramp approach stack below target (2), direct attrition evidence below target (1), redirect explicit threat-swap absent (1), redirect target-swap absent (1), redirect taunt absent (1), Wombo goal peak-share below target (1), and marksman sustained-DPS goal ramp-stack below target (1). The old 6-row effective-health group is now split into sustain, peel, and support-role EHP buckets so tuning can target the right metric path.
- `accepted_gap_kind_summary.csv` now rolls up all 63 accepted spans into 39 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 39 gap kinds and 63 spans, and that every gap kind maps to one or more committed guard scenes.
