...
| Buy XP live-window affordance | The first live attempt exposed stale UI after a raw `Economy.gold` write. The follow-up assist used `Economy.add_gold(...)`, visibly updated to `Gold: 5`, and a real OS Buy XP click updated to `Lvl 2 (2/6)`. `BuyXPTransactionalFeedbackSmoke` guards the committed presenter path: 4g denial leaves labels unchanged and shows `Need +1 gold to buy XP and keep 1 health.`, while a 6g success repaints `Gold: 2` and `Lvl 2 (2/6)`. `PremiumDeployAfterLevelSmoke` adds a Main-scene visible Buy XP success at the first post-opener shop with audit gold only when needed. | Input path covered under valid visible preconditions; reserve-floor denial and valid-success repaint covered mechanically | Natural real-window screenshot of an unassisted Stage 1 Round 3 Buy XP success remains optional; preserve the explicit denial message and immediate progress repaint in future shop UI passes. |
...
Implication:
- Current source and tracked harness evidence prove the model/presenter path can level, expose a cost-2 offer, buy it, and deploy it.
- Natural access to level 2 is variable with reward outcomes and first-shop choices, so it remains play-feel/economy evidence rather than the core correctness gate. The first 4-gold Buy XP denial has explicit visible feedback in the presenter path, while labels correctly remain unchanged.

## Current Natural Buy XP Main-Flow Recheck

Fresh ignored-runner evidence was generated on 2026-06-25 at `outputs/audit_playtest/natural_buy_xp_audit/natural_buy_xp_results.json` using `outputs/audit_playtest/NaturalBuyXPAudit.tscn`. A 2026-06-26 rerun still completed with `NaturalBuyXPAudit: OK` and `errors: []`, but the current JSON is not a clean helper-deploy proof because reward gold allowed immediate Buy XP and the later helper buy/deploy terminal state remained loose. The committed guards are `tests/visual/BuyXPTransactionalFeedbackSmoke.tscn` for Buy XP denial/success feedback and `tests/visual/PremiumDeployAfterLevelSmoke.tscn` for Main-scene level-up into premium purchase/deploy.
...
Implication:
- Ordinary play can naturally reach a valid Buy XP moment, but the exact timing depends on reward and shop outcomes.
- The audit should not use the ignored NaturalBuyXPAudit as cost-2 deployment proof. The immediate communication gap is covered mechanically, and the premium purchase/deploy path is now covered by a committed tracked smoke.
