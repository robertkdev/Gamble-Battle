...
- RoleMatrixSmoke passes all 22 units, but the fresh 2026-06-25 detail recheck found accepted lower-level `FAIL` spans inside aggregate passes. After subject-side filtering and goal-level ramp-applicability cleanup, 21 of 22 reports still contain 85 subject-side accepted spans, with zero non-ramp identities carrying `goal_*_ramp_*` diagnostics.
...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the 2026-06-25 saved report parser found 85 subject-side accepted lower-level `FAIL` spans across 21/22 reports after filtering opponent-side aggregate labels and suppressing non-applicable goal ramp diagnostics. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
The `support_peel_triage` column in `accepted_lower_level_fail_spans.csv` and `support_peel_triage_counts` in `rga_accepted_misses_summary.json` now regenerate these groups directly from current reports. The same export also adds `support_peel_gap_kind` and `support_peel_next_action`. Current gap-kind counts are: counterplay context absent 8, hard-peel submode absent 6, team peel-save metric absent 6, cooldown-trade quality below target 2, CC-prevention context absent 1, peel-interrupt context absent 1, and wombo CC-sync absent 1.
