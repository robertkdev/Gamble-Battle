...
- `tests/visual/FirstShopOfferQualitySamplingSmoke.tscn`: `FirstShopOfferQualitySamplingSmoke: PASS samples=240 bo_good=60/60(1.000) no_good=0 bonko_good=60/60(1.000) no_good=0 cashmere_good=60/60(1.000) no_good=0 repo_good=60/60(1.000) no_good=0`; rerun on 2026-06-26 with `errors: []`, sampling the starter-aware opening-shop roller guard to prove each soft-fail starter sees at least one known advancing helper.
- `tests/visual/AllStarterMainFlowSmoke.tscn`: `AllStarterMainFlowSmoke: PASS starters=12 first_shop=11 retry=1 deployed=11 second_resolved=11`; rerun on 2026-06-26 with `errors: []`, validating every current level-1 starter through the real Main-scene forced opener, first-shop buy/deploy, and second-fight loop while preserving Axiom as the one expected opener retry outlier and asserting Bo, Bonko, Cashmere, and Repo first shops include a known advancing helper.
- `outputs/audit_playtest/AllStarterMainFlowAudit.tscn`: `AllStarterMainFlowAudit: OK starters=12`; rerun on 2026-06-26 with `errors: []` after the starter-aware first-shop guard. The current default first-card replay advanced 9 of 11 first-shop starters beyond Stage 2, left Bo+Brute at Stage 2 with 2 gold, left Korath+Repo at Stage 2 with 1 gold, and kept Axiom as the 2-gold retry starter.
...
- This remains automated behavioral coverage. It does not close the separate live-window screenshot and human drag-feel gaps. Bo, Bonko, Cashmere, and Repo still have visible first-shop choice-quality differences, but the tracked opening-shop safeguard now guarantees each of them sees at least one known advancing helper in the first post-opener shop.
...
- Low-damage defensive pairings can also fail: Repo did not advance with Brute or Korath in either the ignored runner or the latest committed guard.
...
Current 2026-06-26 automated strategy refresh from `AllStarterMainFlowAudit` after the starter-aware first-shop guard:
...
- Clean default first-card advance lines: Berebell+Sari, Bonko+Mortem, Brute+Grint, Cashmere+Bonko, Grint+Korath, Morrak+Grint, Mortem+Berebell, Repo+Bonko, and Sari+Grint advance beyond Stage 2 in the current automated replay.
- Soft-fail/pressure lines: Bo+Brute resolves the second fight but returns to Stage 2 with 2 gold, and Korath+Repo resolves the second fight but returns to Stage 2 with 1 gold. Both had mechanically valid buy/deploy/fight loops, but the default first-card helper did not create a clean early-advance line.
...
- Current starter-aware first-shop sampling guard: over 240 seeded `ShopRoller.roll_opening_for_starter(...)` shops, Bo, Bonko, Cashmere, and Repo each saw at least one known advancing helper in 60/60 samples. This closes random-offer dead ends for those soft-fail starters, but it does not guarantee the first visible/first-clicked card is the strongest helper.
- Tracked first-shop matrix detail now lives in `docs/starter_first_shop_choice_matrix_2026-06-26.md`: latest committed guard telemetry is 10 of 20 advancing forced helper choices plus the starter-aware offer sampling read. Repo's refreshed default line advanced with Bonko, which the forced matrix did not test for Repo, so the guard list is conservative rather than a complete helper taxonomy.
- Current audit interpretation: the mechanical blockers are covered and first-shop availability is guarded; the live design backlog is now default first-card quality and starter pacing, especially Bo seeing Brute first and Korath seeing Repo first despite better advancing helpers being present elsewhere in the shop.
