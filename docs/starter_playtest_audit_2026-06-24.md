...
Follow-up production-clock coverage was added on 2026-06-26 as `tests/visual/ProductionRapidShopPressureSmoke.tscn`. It reuses the same real Main-scene rapid-shop path, but runs at `Engine.time_scale = 1.0`, does not hold opener or shop planning timers, and keeps the extended real-time fight timeouts needed for production clock. The MCP debug run completed with `ProductionRapidShopPressureSmoke: OK purchases=5 deployed_board=6` and `errors: []`. This is still audit-assisted for shop gold and deterministic shop preparation; it closes the accelerated-clock caveat for the tracked pressure path, not the optional physical human-speed feel proof.
...
- The tracked validation now covers both accelerated and production-clock timing for this path. The real-window buying portion is covered for OS-coordinate clicks under a staged Round 2 setup. Remaining risk is physical pointer feel, hit-target clarity, deployment drag feel, and purchase-feedback prominence under normal human play pressure.
