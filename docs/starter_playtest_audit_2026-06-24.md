...
| RGA identity reports | `RoleMatrixSmoke` passes all 22 current units; the saved report parser now finds 6 subject-side accepted lower-level `FAIL` spans across 4/22 reports after filtering opponent-side aggregate labels, suppressing non-applicable goal ramp, soft-peel hard-peel, and non-attrition brawler direct-attrition diagnostics, adding live counterplay scenario rows, demoting AoE low-median rows to diagnostics when alternate max-target or AoE-DPS evidence satisfies the approach, recognizing burst pressure for direct attrition, demoting auxiliary marksman role share rows, demoting alternate marksman side-backline rows when subject ranged/time-on-target evidence proves positioning, demoting low mage share rows when peak-over-mean evidence proves periodicity, demoting low reposition subspans when total path distance satisfies the k-of-n approach, demoting low median displacement or missing first-CC engage subspans when standard or peak displacement plus first-action evidence satisfies the engage approach, demoting Grint's single-target initiate success span when distance plus first-action evidence proves `tank.initiate_fight`, demoting low team-save fallback spans when direct peel/support evidence proves the aggregate, demoting low EHP fallback spans when effective HPS, ally-protection, or support-event evidence proves the aggregate, demoting low counterplay response rows when direct debuff/lockdown evidence proves the aggregate, demoting low frontline/body-block rows when prevention plus frontline presence proves the aggregate, demoting missing redirect submodes when body-block evidence proves `approach_redirect`, demoting low execute bonus-share rows when direct execute bonus damage plus low-HP kill conversion proves `approach_execute`, demoting low ramp-stack rows when direct ramp events plus peak/window evidence prove the aggregate, demoting Wombo's missing third evidence path when the other two direct Wombo paths satisfy the 2-of-3 goal aggregate, and adding dedicated sustained-pressure coverage for marksman sustained-DPS rows. Latest accepted-miss export refresh reports 6 spans across 6 mapped gap kinds, and `AcceptedMissGuardCoverageSmoke` passes. | Covered as smoke; tuning remains open | Treat accepted misses as balance/instrumentation backlog, not starter/shop-flow blocker. |
...
- The saved report JSON now records the subject-side backlog directly: 6 lower-level fail spans, split as 1 role span, 1 approach span, and 4 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 4 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports. Axiom, Berebell, Bo, Bonko, Brute, Cashmere, Grint, Korath, Luna, Morrak, Mortem, Nyxa, Paisley, Repo, Sari, Veyra, Volt, and Vykos are currently the units without a saved lower-level accepted miss.
...
- `accepted_gap_kind_summary.csv` now rolls up all 6 accepted spans into 6 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 6 gap kinds and 6 spans, and that every gap kind maps to one or more committed validation scenes.
