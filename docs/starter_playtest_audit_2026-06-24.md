...
- `tests/rga_testing/validation/AcceptedMissGuardCoverageSmoke.tscn`: `AcceptedMissGuardCoverageSmoke: PASS`; rerun on 2026-06-26 after the marksman sustained scenario-pack pass with `gap_kinds=12`, `accepted_spans=13`, `mapped_gap_kinds=12`, and `errors: []`.
...
- The saved report JSON now records the subject-side backlog directly: 13 lower-level fail spans, split as 2 role spans, 2 approach spans, and 9 goal spans. Future accepted-miss audits should prefer this JSON field over parsing console text or `godot.log`.
- 9 of 22 current units have at least one subject-side accepted lower-level fail span in saved reports. Axiom, Berebell, Bonko, Brute, Korath, Luna, Morrak, Nyxa, Paisley, Repo, Sari, Veyra, and Vykos are currently the units without a saved lower-level accepted miss.
...
- Largest non-support or cross-role gap kinds are pick-burst kill count absent (2), marksman sustained-DPS goal damage share below target (1), direct attrition evidence below target (1), assassin opening presence below target (1), team-fortification buff uptime absent (1), dive backline contact absent (1), and engage success targets below target (1). The old 6-row effective-health group, 5-row frontline/body-block fallback group, 3-row Korath redirect-submode group, 3-row ramp-stack group, 2-row execute bonus-share group, and 2-row Wombo third-path group are now diagnostic fallback coverage instead of accepted-miss debt.
- `accepted_gap_kind_summary.csv` now rolls up all 13 accepted spans into 12 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket. `AcceptedMissGuardCoverageSmoke.tscn` now verifies the regenerated rollup still has 12 gap kinds and 13 spans, and that every gap kind maps to one or more committed validation scenes.
