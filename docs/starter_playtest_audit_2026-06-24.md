...
- Largest non-support or cross-role gap kinds are marksman role damage-share diagnostic below target (5), multi-target coverage below target (5), body-block evidence absent (4), burst approach peak-share below target (4), direct attrition evidence below target (4), backline pressure below target (3), engage CC timing unproven (3), magic damage share below target (3), movement distance below target (3), ramp stack evidence below target (3), redirect threat-swap/taunt absent (3), marksman sustained-DPS goal damage share below target (2), and Wombo goal peak-share below target (1). The old 6-row effective-health group is now split into sustain, peel, and support-role EHP buckets so tuning can target the right metric path.
