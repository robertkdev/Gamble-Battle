...
- Largest non-support or cross-role gap kinds are burst peak share below target (5), marksman role damage-share diagnostic below target (5), multi-target coverage below target (5), body-block evidence absent (4), direct attrition evidence below target (4), backline pressure below target (3), engage CC timing unproven (3), magic damage share below target (3), movement distance below target (3), ramp stack evidence below target (3), redirect threat-swap/taunt absent (3), and marksman sustained-DPS goal damage share below target (2). The old 6-row effective-health group is now split into sustain, peel, and support-role EHP buckets so tuning can target the right metric path.
- `accepted_gap_kind_summary.csv` now rolls up all 80 accepted spans into 33 gap kinds with affected topics, units, labels, block types, and a representative next action. The same detail rows are embedded as `audit_gap_kind_details` in `rga_accepted_misses_summary.json`, while support-specific `support_peel_gap_kind_counts` remains the focused view for the support/peel bucket.
