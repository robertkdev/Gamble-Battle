...
- `tools/art/build_unit_style_drift_audit.py` rebuilds the Vellum/Paisley/token comparison sheets and foreground-detail metrics. Use it whenever a candidate is being promoted to a proof after the style-drift correction. The generated sheets and CSV include each proof's `reference_role` so passing images do not silently become equal anchors.
...
5. Run or update the style-drift audit contact sheet when a candidate is meant to become a new proof. Compare it directly against Vellum first, then Paisley and any narrowly relevant proof examples, for high-detail dry gothic richness, not just non-shiny palette match.
6. Only after the raw image passes style review, run background removal.
7. Review the cutout on checker, black, white, and in a board-scale preview.
8. Do not replace `assets/units/*.png` until the raw and cutout both pass review and the user approves the swap.
...
12. Update the proof ledger with accepted/current/rejected status before handoff.
