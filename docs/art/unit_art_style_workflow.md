...
- The audit measures safety-orange-like residue only in the transparent alpha edge band and soft-alpha pixels; interior orange/gold detail is reported but does not fail by itself. Its manifest must record `audit_input_contract: cutout_rgba_pixels_only` with reference/raw/board/style-anchor image loads all false.
...
- Use this when the full BiRefNet cutout is already good but the strict audit catches any safety-orange edge or soft-alpha residue.
- Perfect exit means `edge_orange_pixels == 0` and `soft_orange_pixels == 0` when rerunning `audit_unit_cutout_orange_fringe.py --strict-zero`, plus a cleaner stats JSON proving `cleaned_safety_orange_pixels` matches the actual cleanup count, `changed_alpha_pixels == 0`, `changed_outside_target_pixels == 0`, `changed_opaque_interior_pixels == 0`, `remaining_edge_orange_pixels == 0`, and `remaining_soft_orange_pixels == 0`.
- The cleaner command prints and writes a JSON stats artifact by default for its own delta contract: target edge-orange pixels, target soft-orange pixels, total target cleanup pixels, changed RGB pixels, changed alpha pixels, changed pixels outside the target, changed opaque interior pixels, remaining edge-orange pixels, remaining soft-orange pixels, and removed cleanup pixels. The JSON must also name the exact input cutout, output cutout, review image, stats file, edge radius, and SHA-256 hashes for the input/output/review files that were audited. Treat any nonzero alpha/outside-target/opaque-interior/remaining-edge-orange/remaining-soft-orange count as a failed clean.
- After post-cleaning, rebuild the board preview and rerun the orange-fringe audit with `--strict-zero` before changing the proof ledger path.
- The standard validation runner includes a synthetic edge-clean regression: a reference-free contaminated cutout must fail the orange-fringe audit first, then pass after `clean_unit_cutout_orange_edge.py` while preserving alpha and intentional interior orange material. The regression also requires the cleaner's default JSON stats path, JSON path provenance, SHA-256 file hashes, self-reported stdout, and actual before/after pixels to agree, and fails if the cleaner changes anything outside safety-orange alpha-edge/soft-alpha target pixels, changes alpha, touches opaque interior pixels, or leaves edge/soft-alpha orange residue behind.
