...
- The standard validation runner includes two synthetic regressions: a reference-free edge/soft contaminated cutout must fail-clean-pass while preserving intentional interior orange material, and a raw-key internal-hole control must pass cutout-only strict audit, fail raw-backed strict audit, then pass after `clean_unit_cutout_orange_edge.py --raw-source` while preserving non-key intentional orange material. These controls are what prevent the Paisley-style inside-hole miss from recurring.
