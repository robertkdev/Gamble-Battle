...
Current playable-unit identity map. Source of truth is `data/units/*.tres` plus the linked `data/identity/unit_identities/*_identity.tres` resources; non-playable creep/test identities under `data/other_units/` are intentionally omitted.
...
| Sari | sari | marksman | marksman.sustained_dps | long_range, debuff, ramp | res://data/identity/unit_identities/sari_identity.tres |
